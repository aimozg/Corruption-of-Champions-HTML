ConsumableEffects.beeTFs = function(type) {
	...
	if ((pure || special) && player.pregnancyType == PregnancyType.FAERIE) { //Pure or special honey can reduce the corruption of a phouka baby
		if (gameFlags[PregnancyType.CORRUPTION] > 1) { //Child is phouka, hates pure honey
     outputText("<br><br>You feel queasy and want to throw up.  There's a pain in your belly and you realize the baby you're carrying didn't like that at all.  Then again, maybe pure honey is good for it.");
		...
		else if (gameFlags[PregnancyType.CORRUPTION] < 1) { //Child is faerie, loves pure honey
     ...
     outputText("<br><br>At first you feel your baby struggle against the honey, then it seems to grow content and enjoy it.");
		...
		gameFlags[PregnancyType.CORRUPTION]--;
...
ConsumableEffects.canineTFs = function(type) {
    ...
    //------------
    if (type == 3) {
        player.dynStats("lib", 2 + rand(4), "lus", 5 + rand(5), "cor", 2 + rand(4));
...
ConsumableEffects.succubiDelight = function(tainted) {
    ...
    if (player.balls < 2 && changes < changeLimit && rand(4) == 0) {
        if (player.balls == 0) {
            player.balls = 2;
            outputText("<br><br>Incredible pain scythes through your crotch, doubling you over. You stagger around, struggling to pull open your " + player.armor.equipmentName.equipmentName + ". In shock, you barely register the sight before your eyes: <b>You have balls!</b>");
            player.ballSize = 1;
        }
        changes++;
    }
...
ConsumableEffects.slimeTFs = function () {
    ...
    if (player.hairType == 3 && (player.skinDesc != "skin" || player.skinAdj != "slimy")) {
        ...
        else if (player.hasScales()) outputText("<br><br>You sigh, feeling slippery wetness over your scales.  You reach to scratch it and come away with a slippery wet coating.  Your scales have transformed into a slimy goop!  Looking closer, you realize your entire body has become far more liquid in nature, and is semi-solid.  Your " + player.armorName + " has even sunk partway into you.");
