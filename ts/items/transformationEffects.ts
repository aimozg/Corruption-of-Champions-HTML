namespace ConsumableEffects {
	...
	export function demonTFs(type: number, purified: boolean) {
	...
	export function equineTFs(): void {
		...
		if (player.skinType == SkinType.FUR && player.faceType == FaceType.HORSE && player.tailType == TailType.HORSE && (player.lowerBody != LowerBodyType.HOOFED)) {
			...
			//Bad End
			if (rand(4) == 0 && gameFlags[HORSE_WARNING] == 1 && !player.hasPerk(PerkLib.TransformationResistance)) {
		...
		if ((player.gender == 1 || player.gender == 3) && rand(1.5) == 0 && changes < changeLimit) {
			...
			if ((player.countCocksOfType(CockTypesEnum.HORSE) + player.countCocksOfType(CockTypesEnum.DEMON)) < player.cocks.length) {
				...
				if (player.cocks.length == 1) {
					...
					temp2 = player.cocks[temp].increaseCock(rand(4) + 4);
					temp3 = 1;
			...
			//Players cocks are all horse-type - increase size!
				...
				//single cock
				if (player.cocks.length == 1) {
					temp2 = player.cocks[0].increaseCock(rand(3) + 1);
					temp  = 0;
					player.dynStats(["sens", 1], ["lust", 10]);
				}
				//Multicock
					...
					//Grow smallest cock!
					//temp2 changes to growth amount
					temp2 = player.cocks[temp].increaseCock(rand(4) + 1);
					player.dynStats(["sens", 1], ["lust", 10]);
				}
				outputText("<br><br>");
		...
		if (player.gender == 2 || player.gender == 3) {
			...
			if (player.statusEffectValue(StatusEffects.Heat, 2, 0) < 30 && rand(2) == 0 && changes < changeLimit) {
				if (player.goIntoHeat(true)) {
					changes++;
				}
			}
	...
	export function slimeTFs(): void {
		...
		if (player.hairType == 3 && (player.skinDesc != "skin" || player.skinAdj != "slimy")) {
			...
			if (player.skinTone != "green" && player.skinTone != "purple" && player.skinTone != "blue" && player.skinTone != "cerulean" && player.skinTone != "emerald") {
				...
				if (player.armType != ArmType.HUMAN || player.clawType != ClawType.NORMAL) restoreArms(tfSource);
			}
