namespace ConsumableEffects {
	...
	//Fish Fillet
	export function fishFillet(): void {
		clearOutput();
		...
		//Blew up factory
		if (gameFlags[FACTORY_SHUTDOWN] == 2) player.dynStats("cor", 0.5);
		//Turned off factory
		else if (gameFlags[FACTORY_SHUTDOWN] == 1) player.dynStats("cor", -0.1);
		//Normal
		else player.dynStats("cor", 0.1);
	...
	//Lust Draft
	export function lustDraft(fuck:boolean=false):void {
		player.slimeFeed();
		clearOutput();
		...
		if (player.lust >= player.maxLust() && !inCombat()) {
			outputText("<br><br>The arousal from the potion overwhelms your senses and causes you to spontaneously orgasm.  You rip off your " + player.armor.equipmentName + " and look down as your ");
	...
	//Vitality Tincture
	export function vitalityTincture(): void {
		player.slimeFeed();
		clearOutput();
	...
	//Scholar's Tea
	export function scholarsTea(): void {
		player.slimeFeed();
	...
	//Hair Dyes
	export function hairDye(newColor:string):void {
	...
	//Skin Oils
	export function skinOil(newColor:string):void {
	...
	//Body Lotions
	export function bodyLotion(newAdj:string):void {
	...
	export function liquidDesc(_adj:string):string {
		switch (_adj) {
			case "smooth":
				return randomChoice(["smooth liquid", "thick cream"]);
			case "rough":
				return randomChoice(["abrasive goop", "rough textured goop"]);
			case "sexy":
				return randomChoice(["smooth liquid", "attractive cream", "beautiful cream"]);
			case "clear":
				return randomChoice(["smooth liquid", "thick cream"]);
			default: //Failsafe
				return randomChoice(["liquid", "cream"]);
		}
	}

	//Tattered Scroll
	export function tatteredScroll(): void {
	...
	// Black Book
	export function blackSpellbook(): void {
		clearOutput();
	...
	// White Book
	export function whiteSpellBook(): void {
		clearOutput();
		...
		//Smart enough for arouse and doesnt have it
		if (player.inte >= 25 && !player.spells.chargeWeapon) {
	...
	// Minotaur Cum
	export function minotaurCum(purified:boolean):void {
		player.slimeFeed();
		clearOutput();
		...
		if (gameFlags[MINOTAUR_CUM_ADDICTION_STATE] == 0 && !player.hasPerk(PerkLib.MinotaurCumResistance)) outputText("potent musk washes over you.");
		else outputText("heavenly scent fills your nostrils.");
		if (!purified) {
			if (gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] < 50) outputText("  It makes you feel dizzy, ditzy, and placid.");
		...
		//(Minotaur fantasy)
		if (!inCombat() == true && rand(10) == 1 && !purified && !player.hasPerk(PerkLib.MinotaurCumResistance)) {
