...
namespace Reducto {
	export function reductoMenu() {
		menu();
		if (player.balls > 0 && player.ballSize > 1) addButton(0, "Balls", reductoBalls);
		if (player.breastRows.length > 0 && player.biggestTitSize() > 0) addButton(1, "Breasts", reductoBreasts);
		if (player.buttRating > 1) addButton(2, "Butt", reductoButt);
		if (player.vaginas.length > 0 && player.clitLength > 0.25) addButton(3, "Clit", reductoClit);
		if (player.cockTotal() > 0 && player.biggestCockArea() > 6) addButton(4, "Cock", reductoCock);
		if (player.hipRating > 2) addButton(5, "Hips", reductoHips);
		if (player.nippleLength > 0.25) addButton(6, "Nipples", reductoNipples);
		if (player.horns > 2) addButton(7, "Horns", shrinkHorns);
	...
	export function reductoBreasts() {
		clearOutput();
