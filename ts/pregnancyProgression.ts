...
namespace pregnancyProgression {
	let statControl     = 0; // This prevents huge stat freakouts while pregnant
	let buttStatControl = 0; // This prevents huge stat freakouts while anal pregnant

	export function updatePregnancy():void {
		...
		if (player.pregnancyType == PregnancyType.AMILY) {
			switch (player.pregnancyEventNum) {
				case 1:
					...
					break;
				case 2:
					outputText("<b>Your belly is getting more noticeably distended and squirming around.  You are probably pregnant.</b><br><br>");
					break;
				case 3:
					...
					outputText("</b><br><br>");
					if (statControl == 0) {
						player.dynStats(["spe", -1],["lib", 1],["sens", 1]);
						player.changeLust(2);
						statControl++;
					}
					break;
			...
			if (player.pregnancyIncubation == 0 && player.pregnancyType == PregnancyType.AMILY) {
			...
			if (player.pregnancyIncubation == 0 && player.pregnancyType == PregnancyType.AMILY) {
				...
				player.orgasm();
				player.dynStats(["str", -1], ["tou", -2], ["spe", 3], ["lib", 1], ["sens", .5]);
				outputText("<br><br>");
		...
		if (player.pregnancyType == PregnancyType.IMP) {
			...
			if (player.pregnancyIncubation <= 72 * 60 && player.pregnancyIncubation >= 48 * 60) {
				...
				if (statControl == 1) {
					statControl = 2;
					player.dynStats(["spe", -3], ["lib", 1], ["sens", 1]);
					player.changeLust(4);
				}
			}
		...
		if (player.pregnancyIncubation == 0 && player.pregnancyType == PregnancyType.IMP) {
			statControl = 0;
			outputText("<br>");
			...
			if (player.gender == 0) player.gender = 2;
			player.orgasm();
			player.dynStats(["tou", -2], ["spe", 2], ["lib", 1], ["sens", .5], ["cor", 7]);
			if (player.buttRating < 10 && rand(2) == 0) {
				...
				player.hipRating++;
				outputText("<br><br>After the birth your " + player.armor.equipmentName + " fits a bit more snugly about your " + player.hipDescript() + ".");
			}
			outputText("<br>");
		}
		...
		if (player.buttPregnancyType == PregnancyType.BEE_EGGS) {
			if (player.buttPregnancyIncubation <= 36 * 60 && player.buttPregnancyIncubation >= 20 * 60) {
				outputText("<b><br>You feel bloated, your bowels shifting uncomfortably from time to time.</b><br><br>");
			}
			if (player.buttPregnancyIncubation <= 20 * 60) {
				outputText("<b><br>A honey-scented fluid drips from your rectum.</b>  At first it worries you, but as the smell fills the air around you, you realize anything with such a beautiful scent must be good.  ");
				...
				outputText("You smile knowing you couldn't stop from masturbating if you wanted to.<br><br>");
		...
		if (player.buttPregnancyIncubation == 0 && player.buttPregnancyType == PregnancyType.BEE_EGGS) {
			outputText("<br>");
			...
			outputText("Each egg seems to come out closer on the heels of the one before, and each time your conscious mind loses more of its ability to do anything but masturbate and wallow in honey.<br><br>");
			...
			if (player.cor >= 90) outputText("You lick your lips, savoring the honeyed residue on them as you admire your thousands of children.  If only every night could be like this...<br>");
			buttStatControl = 0;
			player.buttKnockUpForce(); //Clear Butt Pregnancy
			player.orgasm();
			player.dynStats(["inte", 1], ["lib", 4], ["sens", 3]);
			if (player.buttChange(20, true)) outputText("<br>");
			if (player.buttRating < 17) {
				//Guaranteed increase up to level 10
				if (player.buttRating < 13) {
					player.buttRating++;
					outputText("<br>You notice your " + player.buttDescript() + " feeling larger and plumper after the ordeal.");
				}
		...
		// GOO PREGNANCY
		// --------------
		if (player.pregnancyType == PregnancyType.GOO_GIRL && player.pregnancyIncubation == 0) {
		...
		if (player.buttPregnancyIncubation <= 1 && player.buttPregnancyType == PregnancyType.SANDTRAP_FERTILE) {
			...
			if (player.buttRating < 17) {
				...
				if (player.buttRating < 13) {
					player.buttRating++;
					outputText("<br>You notice your " + player.buttDescript() + " feeling larger and plumper after the ordeal.<br>");
				}
				//Big butts only increase 50% of the time.
				else if (rand(2) == 0) {
					player.buttRating++;
