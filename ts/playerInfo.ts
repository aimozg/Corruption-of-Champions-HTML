...
function attributeMenu(): void {
	...
	if (player.statPoints > 0) {
		if (str < 100) addButton(0, "STR +", addAttribute, "str");
		if (tou < 100) addButton(1, "TOU +", addAttribute, "tou");
		if (spe < 100) addButton(2, "SPE +", addAttribute, "spe");
		if (inte < 100) addButton(3, "INTE +", addAttribute, "inte");
	}
	//Subtract attributes
...
function addAttribute(attribute: AttrName): void {
	switch (attribute) {
		case "str":
			tempStr++;
			break;
...
function subtractAttribute(attribute: AttrName): void {
	switch (attribute) {
		case "str":
			tempStr--;
			break;
...
function resetAttributes(): void {
	...
	player.statPoints += tempStr + tempTou + tempSpe + tempInt;
...
function finishAttributes(): void {
	clearOutput();
	if (tempStr > 0) {
...
function perkConfirmation(): void {
	let perkselect = document.getElementById("perkselect")!! as HTMLInputElement;
	if (perkselect.value == "null") {
		perkBuyMenu();
		return;
	}
	const perkGet = PerkIDs[perkselect.value];
	clearOutput();
...
// PERKS
//------------
function perksScreen(): void {
	clearOutput();
	for (let perk of player.perks) {
		outputText("<b>" + perk.ptype.name + "</b> - " + perk.ptype.desc + "<br>");
	}
	doNext(playerMenu);
	//Additional buttons
	let button = 1;
...
function appearanceScreen(): void {
	...
	//HEAD DESCRIPTION
	//Face
	let faceType                = player.faceType;
	let skinType                = player.skinType;
	let eyeType                 = player.eyeType;
	let earType                 = player.earType;
	let legCount                = player.legCount;
	let buttRating              = player.buttRating;
	let tailVenom               = player.tailVenom;
	let hipRating               = player.hipRating;
	let buttPregnancyType       = player.buttPregnancyType;
	let pregnancyType           = player.pregnancyType;
	let buttPregnancyIncubation = player.buttPregnancyIncubation;
	let pregnancyIncubation     = player.pregnancyIncubation;
	let sLegCount               = num2Text(legCount);
	switch (faceType) {
		case FaceType.HUMAN:
		case FaceType.SHARK_TEETH:
		case FaceType.BUNNY:
		case FaceType.SPIDER_FANGS:
		case FaceType.FERRET_MASK:
			if (skinType == SkinType.PLAIN || skinType == SkinType.GOO)
				outputText(" Your face is human in shape and structure, with " + player.skin() + ".");
			if (skinType == SkinType.FUR)
				outputText(" Under your " + player.skinFurScales() + " you have a human-shaped head with " + player.skin(true, false) + ".");
			if (skinType == SkinType.SCALES)
			...
			else if (faceType == FaceType.BUNNY)
		...
		case FaceType.FERRET:
			...
			break;
		case FaceType.RACCOON_MASK:
			if (skinType != SkinType.FUR && skinType != SkinType.SCALES) {
				outputText(" Your face is human in shape and structure, with " + player.skin());
				if ((player.skinTone == "ebony" || player.skinTone == "black") && (skinType == SkinType.PLAIN || skinType == SkinType.GOO))
				...
				if (((player.furColor == "black" || player.furColor == "midnight") && (skinType == SkinType.FUR || skinType == SkinType.SCALES)))
		...
		case FaceType.RACCOON:
			...
			else if (skinType == SkinType.SCALES)
	...
	if (player.antennae == AntennaeType.BEE) {
		if (earType == EarType.BUNNY) outputText(" Limp antennae also grow from just behind your hairline, waving and swaying in the breeze with your ears.");
	...
	if (player.beardLength > 0) {
		...
		if (player.beardStyle != BeardStyle.GOATEE) {
	...
	switch (player.hornType) {
		...
		case HornType.DRACONIC_X2:
			...
			break;
		case HornType.DRACONIC_X4_12_INCH_LONG:
	...
	if (player.isTaur()) {
		switch (player.lowerBody) {
			case LowerBodyType.HOOFED:
				outputText(" From the waist down you have the body of a horse, with all " + sLegCount + " legs capped by hooves.");
				break;
			case LowerBodyType.PONY:
				outputText(" From the waist down you have an incredibly cute and cartoonish parody of a horse's body, with all " + sLegCount + " legs ending in flat, rounded feet.");
				break;
			case LowerBodyType.DRIDER_LOWER_BODY:
				outputText(" Where your legs would normally start you have grown the body of a spider, with " + sLegCount + " spindly legs that sprout from its sides.");
				break;
			default:
				outputText(" Where your legs would normally start you have grown the body of a feral animal, with all " + sLegCount + " legs.");
				break;
		}
	} else {
	...
	if (player.isTaur()) {
		...
		if (player.tone < 65) {
			...
			if (buttRating >= 6 && buttRating < 10)
				outputText(" is fairly plump and healthy.");
			if (buttRating >= 10 && buttRating < 15)
				outputText(" jiggles a bit as you trot around.");
			if (buttRating >= 15 && buttRating < 20)
	...
	switch (player.tailType) {
		...
		case TailType.FOX:
			if (player.tailCount <= 1)
				...
				outputText(" " + Num2Text(player.tailCount) + " swishing " + player.furColor + " fox's tails extend from your " + player.assDescript() + ", curling around your body - the soft fur feels lovely.");
			break;
	...
	switch (player.lowerBody) {
		case LowerBodyType.HUMAN:
			outputText(" " + Num2Text(legCount) + " normal human legs grow down from your waist, ending in normal human feet.");
			break;
		case LowerBodyType.FERRET:
			outputText(" " + Num2Text(legCount) + " furry, digitigrade legs form below your [hips]. The fur is thinner on the feet, and your toes are tipped with claws.");
			break;
		case LowerBodyType.HOOFED:
			outputText(" Your " + sLegCount + " legs are muscled and jointed oddly, covered in fur, and end in a bestial hooves.");
			break;
		case LowerBodyType.DOG:
			outputText(" " + Num2Text(legCount) + " digitigrade legs grow downwards from your waist, ending in dog-like hind-paws.");
			break;
		...
		case LowerBodyType.DEMONIC_HIGH_HEELS:
			outputText(" Your " + sLegCount + " perfect lissome legs end in mostly human feet, apart from the horn protruding straight down from the heel that forces you to walk with a sexy, swaying gait.");
			break;
		case LowerBodyType.DEMONIC_CLAWS:
			outputText(" Your " + sLegCount + " lithe legs are capped with flexible clawed feet. Sharp black nails grow where once you had toe-nails, giving you fantastic grip.");
			break;
		case LowerBodyType.BEE:
			outputText(" Your " + sLegCount + " legs are covered in a shimmering insectile carapace up to mid-thigh, looking more like a set of 'fuck-me-boots' than exoskeleton. A bit of downy yellow and black fur fuzzes your upper thighs, just like a bee.");
			break;
		...
		case LowerBodyType.CAT:
			outputText(" " + Num2Text(legCount) + " digitigrade legs grow downwards from your waist, ending in soft, padded cat-paws.");
			break;
		case LowerBodyType.LIZARD:
			outputText(" " + Num2Text(legCount) + " digitigrade legs grow down from your " + player.hipDescript() + ", ending in clawed feet. There are three long toes on the front, and a small hind-claw on the back.");
			break;
		case LowerBodyType.BUNNY:
			outputText(" Your " + sLegCount + " legs thicken below the waist as they turn into soft-furred rabbit-like legs. You even have large bunny feet that make hopping around a little easier than walking.");
			break;
		case LowerBodyType.HARPY:
			outputText(" Your " + sLegCount + " legs are covered with " + player.furColor + " plumage. Thankfully the thick, powerful thighs are perfect for launching you into the air, and your feet remain mostly human, even if they are two-toed and tipped with talons.");
			break;
		case LowerBodyType.KANGAROO:
			outputText(" Your " + sLegCount + " furry legs have short thighs and long calves, with even longer feet ending in prominently-nailed toes.");
			break;
		case LowerBodyType.CHITINOUS_SPIDER_LEGS:
			outputText(" Your " + sLegCount + " legs are covered in a reflective black, insectile carapace up to your mid-thigh, looking more like a set of 'fuck-me-boots' than exoskeleton.");
			break;
		case LowerBodyType.FOX:
			outputText(" Your " + sLegCount + " legs are crooked into high knees with hocks and long feet, like those of a fox; cute bulbous toes decorate the ends.");
			break;
		case LowerBodyType.DRAGON:
			outputText(" " + Num2Text(legCount) + " human-like legs grow down from your " + player.hipDescript() + ", sheathed in scales and ending in clawed feet. There are three long toes on the front, and a small hind-claw on the back.");
			break;
		case LowerBodyType.RACCOON:
			outputText(" Your " + sLegCount + " legs, though covered in fur, are humanlike. Long feet on the ends bear equally long toes, and the pads on the bottoms are quite sensitive to the touch.");
			break;
		case LowerBodyType.CLOVEN_HOOFED:
			outputText(" " + Num2Text(legCount) + " digitigrade legs form below your [hips], ending in cloven hooves.");
	}
	...
	if ((buttPregnancyType == PregnancyType.FROG_GIRL) || buttPregnancyType == PregnancyType.SATYR || player.isPregnant()) {
		if (pregnancyType == PregnancyType.OVIELIXIR_EGGS) {
			let sac = player.findStatusEffectByType(StatusEffects.Eggs)!!;
			outputText("<b>");
			//Compute size
			let temp = sac.value3 + sac.value2 * 10;
			...
			} else if (pregnancyIncubation <= 20) {
				outputText("Your swollen pregnant belly is as large as a ");
				if (temp < 10)
					outputText("watermelon.");
				if (temp >= 10 && temp < 20)
			...
			if (buttPregnancyIncubation < 125 && buttPregnancyIncubation >= 75) {
				...
				if (InCollection(pregnancyType,
						PregnancyType.IZMA, PregnancyType.MOUSE, PregnancyType.AMILY, PregnancyType.EMBER,
						PregnancyType.BENOIT, PregnancyType.COTTON, PregnancyType.URTA, PregnancyType.BEHEMOTH)
					|| (pregnancyType == PregnancyType.JOJO && (gameFlags[JOJO_CORRUPTION_STAGE] <= 0 || gameFlags[JOJO_BIMBO_STATE] >= 3)))
					outputText("<br><b>Your belly protrudes unnaturally far forward, bulging with the spawn of one of this land's natives.</b>");
				else if (pregnancyType != PregnancyType.MARBLE)
			...
			if (pregnancyIncubation <= 144 && pregnancyIncubation > 72) {
			...
			if (pregnancyIncubation <= 72 && pregnancyIncubation > 48) {
				outputText("<b>Your stomach is painfully distended by your pregnancy, making it difficult to walk normally.</b>");
			}
			if (pregnancyIncubation <= 48) {
				outputText("<br><b>Your belly protrudes unnaturally far forward, bulging with the spawn of one of this land's natives.</b>");
			}
		} else if (buttPregnancyType == PregnancyType.FROG_GIRL) {
			if (buttPregnancyIncubation >= 8)
			...
			if (pregnancyIncubation <= 336 && pregnancyIncubation > 280) {
				outputText("<b>Your belly is larger than it used to be.</b>");
			} else if (pregnancyIncubation <= 280 && pregnancyIncubation > 216) {
				outputText("<b>Your belly is more noticably distended.  You are probably pregnant.</b>");
			} else if (pregnancyIncubation <= 216 && pregnancyIncubation > 180) {
				outputText("<b>The unmistakable bulge of pregnancy is visible in your tummy.</b>");
			} else if (pregnancyIncubation <= 180 && pregnancyIncubation > 120) {
				outputText("<b>Your belly is very obviously pregnant to anyone who looks at you.</b>");
			} else if (pregnancyIncubation <= 120 && pregnancyIncubation > 72) {
				outputText("<b>It would be impossible to conceal your growing pregnancy from anyone who glanced your way.</b>");
			} else if (pregnancyIncubation <= 72 && pregnancyIncubation > 48) {
				outputText("<b>Your stomach is painfully distended by your pregnancy, making it difficult to walk normally.</b>");
			} else if (pregnancyIncubation <= 48) { //Surely Benoit and Cotton deserve their place in this list
				if (InCollection(pregnancyType,
						PregnancyType.IZMA, PregnancyType.MOUSE, PregnancyType.AMILY, PregnancyType.EMBER,
						PregnancyType.BENOIT, PregnancyType.COTTON, PregnancyType.URTA, PregnancyType.MINERVA,
						PregnancyType.BEHEMOTH)
					|| (pregnancyType == PregnancyType.JOJO && gameFlags[JOJO_CORRUPTION_STAGE] <= 0))
					outputText("<br><b>Your belly protrudes unnaturally far forward, bulging with the spawn of one of this land's natives.</b>");
				else if (pregnancyType != PregnancyType.MARBLE)
					outputText("<br><b>Your belly protrudes unnaturally far forward, bulging with the unclean spawn of some monster or beast.</b>");
				else outputText("<br><b>Your belly protrudes unnaturally far forward, bulging outwards with Marble's precious child.</b>");
	...
	let breastRows = player.breastRows;
	if (breastRows.length == 0) {
		outputText("You have " + num2Text(breastRows.length) + " rows of breasts, which is pretty paradoxical.");
		//Done with tits.  Move on.
		outputText("<br>");
	} else if (breastRows.length == 1) {
		let row = breastRows[0];
		outputText("You have " + num2Text(row.breasts) + " " + player.breastDescript(0) + ", each supporting ");
		outputText(num2Text(row.nipplesPerBreast) + " "); //Number of nipples.
		if (useMetrics)
			outputText(Math.floor(row.nippleLength * 2.54 * 10) / 10 + "-cm "); //Centimeter display
		else
			outputText(Math.floor(row.nippleLength * 10) / 10 + "-inch "); //Inches display
		outputText(player.nippleDescript(0) + (row.nipplesPerBreast == 1 ? "." : "s.")); //Nipple description and plural
		if (row.milkFullness > 75)
			outputText(" Your " + player.breastDescript(0) + " are painful and sensitive from being so stuffed with milk. You should release the pressure soon.");
		if (row.breastRating >= 1)
			outputText(" You could easily fill a " + player.breastCup(0) + " bra.");
		//Done with tits.  Move on.
		outputText("<br>");
	}
	//many rows
	else {
		outputText("You have " + num2Text(breastRows.length) + " rows of breasts, the topmost pair starting at your chest.");
		let breastListText = "";
		breastListText += "<ul>";
		for (let i = 0; i < breastRows.length; i++) {
			breastListText += "<li>";
			if (i == 0) breastListText += "Your uppermost rack houses ";
			if (i == 1) breastListText += "The second row holds ";
			if (i == 2) breastListText += "Your third row of breasts contains ";
			if (i == 3) breastListText += "Your fourth set of tits cradles ";
			if (i == 4) breastListText += "Your fifth and final mammory grouping swells with ";
			let breastRow = breastRows[i];
			breastListText += num2Text(breastRow.breasts) + " " + player.breastDescript(i) + " with ";
			breastListText += num2Text(breastRow.nipplesPerBreast) + " "; //Number of nipples per breast
			if (useMetrics)
				breastListText += Math.floor(breastRow.nippleLength * 2.54 * 10) / 10 + "-cm "; //Centimeter
			else
				breastListText += Math.floor(breastRow.nippleLength * 10) / 10 + "-inch "; //Inches
			breastListText += player.nippleDescript(i) + (breastRow.nipplesPerBreast == 1 ? " each." : "s each."); //Description and Plural
			if (breastRow.breastRating >= 1)
				breastListText += " They could easily fill a " + player.breastCup(i) + " bra.";
			if (breastRow.milkFullness > 75)
				breastListText += " Your " + player.breastDescript(i) + " are painful and sensitive from being so stuffed with milk. You should release the pressure soon.";
			breastListText += "</li>";
		}
		breastListText += "</ul>";
	...
	if (player.cocks.length == 1) {
		let cock = player.cocks[0];
		if (player.isTaur())
			outputText("<br>Your equipment has shifted to lie between your hind legs, like a feral animal.");
		if (useMetrics)
			outputText("<br>Your " + player.cockDescript(0) + " is " + Math.round(cock.cockLength * 10 * 2.54) / 10 + " cm long and ");
		else
			outputText("<br>Your " + player.cockDescript(0) + " is " + Math.round(cock.cockLength * 10) / 10 + " inches long and ");
		let thickness = cock.cockThickness;
		if (Math.round(10 * thickness) / 10 < 10) {
			if (useMetrics) {
				if (Math.round(thickness * 10 * 2.54) / 10 == 1)
					outputText(Math.round(thickness * 10 * 2.54) / 10 + " centimetre thick.");
				else
					outputText(Math.round(thickness * 10 * 2.54) / 10 + " centimetres thick.");
			} else {
				if (Math.round(thickness * 10) / 10 == 1)
					outputText(Math.round(thickness * 10) / 10 + " inch thick.");
				else
					outputText(Math.round(thickness * 10) / 10 + " inches thick.");
			}
		} else outputText(num2Text(Math.round(10 * thickness) / 10) + " inches wide.");
		...
		switch (cock.cockType) {
			...
			case CockTypesEnum.FOX:
				if (cock.knotMultiplier >= 1.8)
					outputText(" The obscenely swollen lump of flesh near the base of your " + player.cockDescript(0) + " looks almost too big for your cock.");
				else if (cock.knotMultiplier >= 1.4)
					outputText(" A large bulge of flesh nestles just above the bottom of your " + player.cockDescript(0) + ", to ensure it stays where it belongs during mating.");
				else if (cock.knotMultiplier > 1)
					outputText(" A small knot of thicker flesh is near the base of your " + player.cockDescript(0) + ", ready to expand to help you lodge it inside a female.");
				//List thickness
				outputText(" The knot is " + Math.round(thickness * cock.knotMultiplier * 10) / 10 + " inches wide when at full size.");
				break;
	...
	if (player.balls > 0) {
		if (player.hasStatusEffect(StatusEffects.Uniball)) {
			if (skinType != SkinType.GOO)
				outputText("Your " + player.sackDescript() + " clings tightly to your groin, holding " + player.ballsDescript() + " snugly against you.");
			else if (skinType == SkinType.GOO)
				outputText("Your " + player.sackDescript() + " clings tightly to your groin, dripping and holding " + player.ballsDescript() + " snugly against you.");
	...
	if (player.vaginas.length > 0) {
		let wet   = player.vaginas[0].vaginalWetness;
		let loose = player.vaginas[0].vaginalLooseness;
		...
		if (player.vaginas.length == 1) {
			if (useMetrics)
				outputText("You have a " + player.vaginaDescript(0) + ", with a " + Math.floor(player.clitLength * 10 * 2.54) / 10 + "-centimetre clit");
			else
				outputText("You have a " + player.vaginaDescript(0) + ", with a " + Math.floor(player.clitLength * 10) / 10 + "-inch clit");
		...
		if (player.vaginas.length > 1) {
			if (useMetrics)
				outputText("You have " + player.vaginas.length + " " + player.vaginaDescript(0) + "s, with " + Math.floor(player.clitLength * 10 * 2.54) / 10 + "-centimetre clits each. ");
			else
				outputText("You have " + player.vaginas.length + " " + player.vaginaDescript(0) + "s, with " + Math.floor(player.clitLength * 10) / 10 + "-inch clits each. ");
		}
		let wetflag = false;
		if (player.lib < 50 && player.lust < 50) { //not particularly horny
			if (wet >= VAGINA_WETNESS_WET) {
				//Wetness
				if (wet < VAGINA_WETNESS_DROOLING) outputText("Moisture gleams in ");
				else outputText("Occasional beads of lubricant drip from ");
				wetflag = true;
			}
		} else if (player.lib < 80 && player.lust < 80) { //kinda horny
			//Wetness
			if (wet < VAGINA_WETNESS_WET) outputText("Moisture gleams in ");
			else if (wet < VAGINA_WETNESS_DROOLING) outputText("Occasional beads of lubricant drip from ");
			wetflag = true;
		} else { //WTF horny!
			//Wetness
			if (wet < VAGINA_WETNESS_WET) outputText("Occasional beads of lubricant drip from ");
			else if (wet < VAGINA_WETNESS_DROOLING) outputText("Thin streams of lubricant occasionally dribble from ");
			else if (wet >= VAGINA_WETNESS_DROOLING) outputText("Thick streams of lubricant drool constantly from ");
			wetflag = true;
		}
		if (wetflag) {
			//Different description based on vag looseness
			if (loose < VAGINA_LOOSENESS_LOOSE) outputText("your " + player.vaginaDescript(0) + ". ");
			else if (loose < VAGINA_LOOSENESS_GAPING_WIDE) outputText("your " + player.vaginaDescript(0) + ", its lips slightly parted. ");
			else outputText("the massive hole that is your " + player.vaginaDescript(0) + ". ");
		}
