...
function battleMenu(): void {
	...
	if (player.hasStatusEffect(StatusEffects.Bind)) {
		menu();
...
function tease(justText: boolean = false): void {
	//Go on!
	...
	/*if (player.hasStatusEffect(StatusEffects.Sealed) && player.statusEffectValue(StatusEffects.Sealed, 2) == 1) {
...
function flee(callHook:boolean=false):void {
	...
	/*if (inCombat() && player.hasStatusEffect(StatusEffects.Sealed) && player.statusEffectValue(StatusEffects.Sealed, 2) == 4) {
	...
	if ((monster instanceof SandTrap) && SandTrap.trap >= 0 && player.canFly()) {
		clearOutput();
	...
	/*if (monster.hasStatusEffect(StatusEffects.GenericRunDisabled)) {
		outputText("You can't escape from this fight!");
		success = null;
	}*/
	if (monster instanceof SandTrap && SandTrap.trapLevel() < 4) {
	...
	if (player.tailType == TailType.RACCOON && player.earType == EarType.RACCOON && player.hasPerk(PerkLib.Runner)) escapeMod -= 25;
	...
	if (monster instanceof Anemone) {
		...
		//Speed dependent
			...
			//Run failed:
				...
				monster.applyVenom((4 + player.sens / 20));
				success = false;
			}
		}
	}
	...
	if (success !== undefined && player.spe > rand(monster.spe + escapeMod)) {
		...
		else if (player.tailType == TailType.RACCOON && player.earType == EarType.RACCOON && player.hasPerk(PerkLib.Runner)) //sekrit benefit: if you have coon ears, coon tail, and Runner perk, change normal Runner escape to flight-type escape
	...
	//FAIL FLEE
		...
		else if (player.tailType == TailType.RACCOON && player.earType == EarType.RACCOON && player.hasPerk(PerkLib.Runner)) // >>>>>>[P] FAIL
...
function wait():void {
	...
	if (monster.refName == "sandtrap") {
		...
		let s = player.findStatusEffectByType(StatusEffects.Bind);
		if (s) {
				switch (s.value1) {
					case BIND_TYPE_GOO:
					...
					case BIND_TYPE_NAGA:
						...
						player.changeLust(player.sens / 5 + 5, true, false);
						break;
					default:
				}
			}
...
function struggle():void {
	clearOutput();
	let damage = 0;
	switch (player.statusEffectValue(StatusEffects.Bind, 1, -1)) {
		case BIND_TYPE_GOO:
...
function fatigueRecovery():void {
	...
	//if (player.hasPerk(PerkLib.SpeedyRecovery)) player.changeFatigue(-1, false);
	//if (player.hasPerk(PerkLib.EnlightenedNinetails) || player.hasPerk(PerkLib.CorruptedNinetails)) player.changeFatigue(-(1 + rand(3)), false);
}
function teaseXP(XP:number):void {
	while (XP > 0) {
		XP--;
		player.teaseXP++;
		//Level dat shit up!
...
function combatRoundOver(skipEnemy:boolean=false):boolean {
...
function checkCombatOver():boolean {
...
function cleanupAfterCombat(nextFunc:null|(()=>void)=null):void {
	if (monster.HP <= 0 || monster.lust >= monster.maxLust()) {
	...
	player.clearStatuses();
	monster.drops          = [];
	monster.dropThresholds = [];
}

function awardPlayer(nextFunc:()=>void):void {
...
function gameOver():void {
	...
	addButton(0, "Game Over", mainMenu, null, null, null, "Your game has ended. Please load a game or start a new game.");
