...
class Amily extends Monster {
	...
	constructor() {
		...
		this.gems       = 5 + rand(5);
		//Battle variables
		this.weapon     = Items.monsterWeapon("knife", 6, "slash");
		this.armor      = Items.monsterArmor("rags", 1);
		this.lustVuln   = 1;
		//Appearance
		this.tallness   = 48;
...
namespace AmilyScene {
	addToGameFlags(
		AMILY_MET, AMILY_MET_AS, AMILY_PC_GENDER, AMILY_OFFER_ACCEPTED, AMILY_AFFECTION, AMILY_OFFERED_DEFURRY,
		AMILY_FUCK_COUNTER, AMILY_NOT_FURRY, AMILY_WANG_LENGTH, AMILY_PREGNANCY_TYPE, AMILY_INCUBATION,
		AMILY_BUTT_PREGNANCY_TYPE, AMILY_OVIPOSITED_COUNTDOWN, AMILY_GROSSED_OUT_BY_WORMS, AMILY_FOLLOWER,
		AMILY_ALLOWS_FERTILITY, FOLLOWER_AT_FARM_AMILY, AMILY_CORRUPT_FLIPOUT, AMILY_TIMES_FUCKED_FEMPC,
		AMILY_VILLAGE_ENCOUNTERS_DISABLED, AMILY_CONFESSED_LESBIAN, AMILY_WANG_LENGTH, AMILY_WANG_GIRTH,
		AMILY_HERM_TIMES_FUCKED_BY_FEMPC, AMILY_HERM_QUEST, PC_TIMES_BIRTHED_AMILYKIDS, AMILY_CORRUPT_FLIPOUT,
		AMILY_VISITING_URTA, CREATE_POTENT_MIXTURE, AMILY_BIRTH_TOTAL, AMILY_TREE_FLIPOUT
	);
	addToGameStrings(AMILY_CLOTHING);
	
	export let amilypreg = new Amily(); // Used for Pregnancy tracking
	
	let sexForced = false; // Used to get around a nasty bug.
	
	
	// Add a pregnancy array OLD CODE
	//pregnancy = new PregnancyStore.Pregnancy(gameFlags[AMILY_PREGNANCY_TYPE], gameFlags[AMILY_INCUBATION], gameFlags[AMILY_BUTT_PREGNANCY_TYPE], gameFlags[AMILY_OVIPOSITED_COUNTDOWN]);
	
	// Add a pregnancy event array NEW CODE
	amilypreg.eventFill(INCUBATION_AMILY_EVENT);
	...
	export function start(): void {
		...
		if (gameFlags[AMILY_CORRUPT_FLIPOUT] > 0 && player.cor > 25) {
			...
			if (player.hasKeyItem(KeyItems.PotentMixture) && gameFlags[CREATE_POTENT_MIXTURE] < 3) {
				cookAmilyASnack();
				return;
			}
			else {
				if (player.hasKeyItem(KeyItems.PotentMixture) && gameFlags[AMILY_CORRUPTION_PATH] == 0) amilyCorrupt1();
				else if (player.hasKeyItem(KeyItems.PotentMixture) && gameFlags[AMILY_CORRUPTION_PATH] == 1) amilyCorrupt2();
				else if (player.hasKeyItem(KeyItems.PotentMixture) && gameFlags[AMILY_CORRUPTION_PATH] == 2) amilyCorrupt3();
		...
		if (amilypreg.isPregnant() == true && amilypreg.pregnancyIncubation == 0) {
			amilyGivesBirth();
			amilypreg.knockUpForce(0, 0); //Clear Pregnancy
			return;
		}
		// SOMETHING IS FUCKED HERE
		
		// Transformation Scene
		if (amilyCanHaveTFNow() == true) {
			amilyDefurrify();
			return;
		}
		...
		if (gameFlags[AMILY_PC_GENDER] == player.gender) {
			...
			if (gameFlags[AMILY_BIRTH_TOTAL] + gameFlags[PC_TIMES_BIRTHED_AMILYKIDS] >= 5 && gameFlags[AMILY_VILLAGE_ENCOUNTERS_DISABLED] == 0) {
				if (gameFlags[AMILY_AFFECTION] < 40) amilyBadEnding();
				else amilyBecomesFollower();
	...
	function amilyStandardMeeting(): void {
		...
		switch (amilypreg.pregnancyEventNum) {
			case 1:
			case 2:
			case 3:
			case 4:
	...
	function hermRenegotiate(): void {
		...
		// TODO if (gameFlags[AMILY_NOT_FURRY] == 0) addButton(2, "No Furries", null, null, null, "Add Tooltip Later.");
		
	}
	
	// Agree to Amily's hermness. - COMPLETE
	function amilyAgreeHerm(): void {
	...
	function amilyConversationStart(): void {
		clearOutput();
	...
	function talkToAmilyWithSexAfter(): void {
		sexForced = true;
	...
	function amilySexHappens(): void {
		...
		else if (gameFlags[AMILY_FUCK_COUNTER] == 0) {
		...
		//Moderate Affection Sex:
			...
			if (amilypreg.pregnancyEventNum >= 1 && amilypreg.pregnancyEventNum <= 5) outputText("  However, she does sometimes touch the swell signifying the litter growing inside her, and when she does her attitude becomes uncertain and nervous.");
			...
			if (amilypreg.pregnancyEventNum >= 6) outputText("  However, her confidence visibly slips when she has to fully bare the bulging belly that marks her pregnant state, but she musters the confidence and starts to show it off for you as well.");
			...
			if (amilypreg.pregnancyEventNum >= 6) fuckAmilyPreg();
			//else
	...
	function determineAmilySexEvent(): void {
		...
		if (gameFlags[AMILY_CONFESSED_LESBIAN] > 0 && player.gender == 2) {
			if (gameFlags[AMILY_WANG_LENGTH] > 0) {
				...
				if (!amilypreg.isPregnant()) amilyHermOnFemalePC();
				//else 50/50
				else {
					if (rand(2) == 0) girlyGirlMouseSex();
					else amilyHermOnFemalePC();
				}
			}
			//LESBO LUVIN!
			else girlyGirlMouseSex();
		}
		...
		if (player.gender == 3 && gameFlags[AMILY_HERM_QUEST] == 2) {
			...
			if (gameFlags[AMILY_WANG_LENGTH] > 0) {
				...
				//Amily is preg
					...
					//Pc is not
					if (!player.isPregnant()) amilyHermOnFemalePC();
					//PC is preg too!
					else {
						//Herm Amily knocks up PC
						if (rand(2) == 0) amilyHermOnFemalePC();
			...
			//Amily still girl!
				...
				if (!player.isPregnant()) sexWithAmily();
		...
		if (player.gender == 1) {
			if (sexForced == true) amilySexHappens();
			else sexWithAmily();
		}
	...
	function amilySexCaressHer(): void {
		clearOutput();
	...
	function amilySexRefrainKiss(): void {
		clearOutput();
	...
	// COMPLETE
	function amilySexEnjoyShow(): void {
		clearOutput();
		...
		if (amilypreg.pregnancyEventNum >= 6) outputText("  Even though she was clearly a little nervous about her gravid state in the beginning, as she continues, she grows in confidence to the point it seems she has almost forgotten about it.");
		outputText("<br><br>");
		amilySexMidPartII();
	}
	
	// COMPLETE
	function amilySexMidPartII(): void {
		player.changeLust(5);
	...
	// COMPLETE
	function amilySexMidPartIII(): void {
		player.changeLust(5);
	...
	// COMPLETE
	function amilySexPlayAlong(): void {
		clearOutput();
	...
	// COMPLETE
	function amilySexWorkToPlease(): void {
		clearOutput();
	...
	function amilySexMidPartIV(): void {
	...
	// COMPLETE
	function amilySexSayGoodbye(): void {
	...
	// COMPLETE
	function amilySexStayAwhile(): void {
	...
	function amilyHighAffectionSex(): void {
		...
		if (amilypreg.isPregnant()) outputText("  Even in a slightly pregnant state, she goes surprisingly fast, though she's also rather cautious of her small bump.");
		outputText("<br><br>");
		...
		if (amilypreg.isPregnant()) outputText("  Really it seems the only thing between you two now is Amily's small stomach bulge.");
		...
		if (amilypreg.isPregnant()) outputText("  As you move your head across her beautiful form, you stop at her growing baby bump and give it a small kiss.");
		outputText("  Your head hovers at her breasts");
		if (amilypreg.isPregnant()) outputText(", which seem to have grown from the pregnancy,");
		
		outputText(" and get to work licking and suckling her nipples, rubbing the sensitive mounds.");
		if (amilypreg.isPregnant()) outputText("  When you get a dribble of milk in your mouth it surprises you, but you certainly don't stop.  \"<i>Hah...You're going to have to teach the children how that's done,</i>\" Amily says, in between fevered breaths.");
		outputText("<br><br>");
		...
		if (amilypreg.isPregnant()) outputText("  You are of course mindful of her baby bump, not going too fast and making sure you're in a position that's comfortable for the two of you, not wanting to harm your future offspring with the lovely mouse maiden.");
		...
		if (!amilypreg.isPregnant()) outputText("Fill me up with everything you have... I want to be a mother for your children, just as much as I want to be a mother of my own people,");
		...
		if (!amilypreg.isPregnant()) outputText("\"<i>If that didn't knock me up... I don't care as much as you'd think. It was magnificent either way,");
		...
		if (!amilypreg.isPregnant()) outputText("Okay... Well, I'm sure you'll be back. I will need your help again if this doesn't set,");
	...
	// COMPLETE
	function amilyHermOnFemalePC(): void {
	...
	function amilySexFirstTime(): void {
		clearOutput();
	...
	function amilySexFirstTimeTakeCharge(): void {
		clearOutput();
	...
	function amilySexFirstTimeHesitate(): void {
		clearOutput();
		//outputText(images.showImage("amily-forest-plainfuck"));
	...
	function amilySexFirstTimeKiss(): void {
		clearOutput();
	...
	function amilyPreggoChance(): void {
		...
		if (rand(4) == 0 || player.cumQ() > rand(1000)) {
			...
			amilypreg.knockUpForce(PregnancyType.PLAYER, INCUBATION_MOUSE - 182, INCUBATION_AMILY_EVENT);
			
		}
	}
	
	// COMPLETE
	function amilyGivesBirth(): void {
		...
		menu();
		addButton(0, "Leave", amilyLaborLeave, null, null, null, "Tooltip to be added.");
		addButton(1, "Watch", amilyLaborWatch, null, null, null, "Tooltip to be added.");
		addButton(2, "Help", amilyLaborHelp, null, null, null, "Tooltip to be added.");
	}
	
	// COMPLETE
	function amilyLaborLeave(): void {
		clearOutput();
	...
	function postBirthingEndChoices(): void {
		...
		gameFlags[PC_TIMES_BIRTHED_AMILYKIDS]++;
		menu();
		addButton(0, "Accept", acceptAmilyHermPath, null, null, null, "TO BE ADDED");
		addButton(1, "Stay Friends", declineAmilyHermPath, null, null, null, "TO BE ADDED");
		addButton(2, "Shoot Down", rejectAmilyHermPath, null, null, null, "TO BE ADDED");
		
	}
	
	function acceptAmilyHermPath(): void {
		clearOutput();
		...
		gameStrings[AMILY_CLOTHING]                  = "rags";
		doNext(playerMenu); // Check this.
	}
	
	//[=Stay Friends=]
	function declineAmilyHermPath(): void {
		clearOutput();
	...
	function rejectAmilyHermPath(): void {
	...
	function amilyCorrupt3(): void {
		...
		//RAPE 3 GO
		amilyCorruptionRape(); //chooseYourAmilyRape();
	}
	
	//COMPLETE
	function amilyCorruptionRape(): void {
		...
		//2nd rape scene
			...
			if (player.gender == 1) doNext(rapeCorruptAmily2Male);
				...
				menu();
				addButton(0, "Male Focus", rapeCorruptAmily2Male, null, null, null, "To be added");
				addButton(1, "Female Focus", rapeCorruptAmily2Female, null, null, null, "To be added");
			}
		}
		//3nd rape scene
			...
			if (player.gender == 1) doNext(rapeCorruptAmily3Male);
				...
				menu();
				addButton(0, "Male Focus", rapeCorruptAmily3Male, null, null, null, "To be added");
				addButton(1, "Female Focus", rapeCorruptAmily3Female, null, null, null, "To be added");
			}
		}
	...
	//OMPLETE
	export function rapeCorruptAmily1(): void {
		...
		if (player.gender == 3) {
			...
			menu();
			addButton(0, "Cock", rapeCorruptAmily1Male, null, null, null, "To be written");
			addButton(1, "Pussy", rapeCorruptAmily1Female, null, null, null, "To be written");
	...
	function rapeCorruptAmily4Meeting(): void {
		...
		if (player.gender == 3) {
			...
			//[Cock] [Pussy]
			menu();
			addButton(0, "Cock", rapeCorruptAmily4Male, null, null, null, "To be added");
			addButton(1, "Pussy", rapeCorruptAmily4Female, null, null, null, "To be added");
		}
	...
	function rapeCorruptAmily4Epilogue(): void {
		...
		gameFlags[AMILY_VAGINAL_WETNESS] = 1;
		gameStrings[AMILY_CLOTHING]      = "sexy rags";
		/*
		 //}
	...
	function amilyDefurrify(): void {
		...
		if (amilyCorrupt()) {
			...
			else player.destroyItems(Items.Consumables.SuccubiMilkPurified, 1);
		} else player.destroyItems(Items.Consumables.SuccubiMilkPurified, 1);
	...
	function amilyBecomesFollower(): void {
		clearOutput();
