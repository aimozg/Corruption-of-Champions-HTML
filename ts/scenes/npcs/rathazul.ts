namespace RathazulScene {
	...
	function returnToRathazulMenu(): void {
		if (gameFlags[RATHAZUL_CAMP] > 0)
			campRathazul();
		else
			encounterRathazul();
	...
	function rathazulWorkOffer(): boolean {
		...
		if (player.hasItem(Items.Consumables.BlackEgg) || player.hasItem(Items.Consumables.LBlackEgg)) {
		...
		/*if (player.hasItem(Items.Materials.SpiderSilk) && flags[RATHAZUL_SILK_ARMOR_COUNTDOWN] + flags[UNKNOWN_FLAG_NUMBER_00275] == 0) { //SPOIDAH
		 showArmorMenu = true;
		 totalOffers++;
		 outputText("\"<i>Oooh, is that some webbing from a giant spider or spider-morph? Most excellent! With a little bit of alchemical treatment, it is possible I could loosen the fibers enough to weave them into something truly magnificent - armor, or even a marvelous robe,</i>\" offers <br><br>");
		 }
		...
		//Marae bark armor
		//if (player.hasKeyItem(KeyItems."Tentacled Bark Plates") >= 0 || player.hasKeyItem("Divine Bark Plates") >= 0) showArmorMenu = true;
		...
		if (gameFlags[RATHAZUL_CAMP] > 0) {
			/*if (flags[RATHAZUL_DEBIMBO_OFFERED] == 0 && (sophieBimbo.bimboSophie() || player.hasPerk(PerkLib.BroBrains) || player.hasPerk(PerkLib.BimboBrains) || player.hasPerk(PerkLib.FutaFaculties))) {
		...
		if (totalOffers > 0) {
			...
			if (dyes) {
				addButton(1, "Buy Dye", buyDyes, null, null, null, "Ask him to make a dye for you. <br><br>Cost: 50 Gems.");
				addButton(2, "Buy Oil", buyOils, null, null, null, "Ask him to make a skin oil for you. <br><br>Cost: 50 Gems.");
				addButton(3, "Buy Lotion", buyLotions, null, null, null, "Ask him to make a body lotion for you. <br><br>Cost: 50 Gems.");
			}
			if (purify) addButton(4, "Purify", purifySomething, null, null, null, "Ask him to purify any tainted potions. <br><br>Cost: 20 Gems.");

			/*if (debimbo) addButton(5, "Debimbo", makeADeBimboDraft, null, null, null, "Ask Rathazul to make a debimbofying potion for you. <br><br>Cost: 250 Gems <br>Needs 5 Scholar Teas.");
			 if (player.hasItem(Items.Consumables.BeeHoney)) addButton(6, Items.Consumables.PureHoney.shortName, rathazulMakesPureHoney, null, null, null, "Ask him to distill a vial of bee honey into a pure honey. <br><br>Cost: 25 Gems <br>Needs 1 vial of Bee Honey");
			 if (gameFlags[RATHAZUL_PURCHASE_COUNTER] >= 5) addButton(7, "ProLactaid", rathazulMakesMilkPotion, null, null, null, "Ask him to brew a special lactation potion. <br><br>Cost: 250 Gems <br>Needs 5 Lactaids and 2 Purified LaBovas.");
			 if (gameFlags[RATHAZUL_PURCHASE_COUNTER] >= 5) addButton(8, "Taurinum", rathazulMakesTaurPotion, null, null, null, "Ask him to brew a special potion that could aid in becoming a centaur. <br><br>Cost: 100 Gems <br>Needs 2 Equinum and 1 Minotaur Blood.");
	...
	function craftCarapace(): void {
		...
		if (player.biggestTitSize() >= 8) outputText("Your " + Appearance.biggestBreastSizeDescript(player) + " barely fit into the breastplate, leaving you displaying a large amount of jiggling cleavage.<br><br>");
	...
	function commissionSilkArmorForReal(): void {
		...
		menu();
		if (player.hasItem(Items.Materials.SpiderSilk, 5)) {
			addButton(0, "Armor", chooseArmorOrRobes, 1, null, null, Items.Armor.SpiderSilkArmor.description);
			addButton(1, "Robes", chooseArmorOrRobes, 2, null, null, Items.Armor.SpiderSilkRobes.description);
		}
		addButton(2, "Bra", chooseArmorOrRobes, 3, null, null, Items.Undergarments.SpiderSilkBra.description);
		addButton(3, "Panties", chooseArmorOrRobes, 4, null, null, Items.Undergarments.SpiderSilkPanties.description);
		addButton(4, "Loincloth", chooseArmorOrRobes, 5, null, null, Items.Undergarments.SpiderSilkLoincloth.description);
	...
	function chooseArmorOrRobes(armorType:"armor"|"robes"|"bra"|"panties"|"loincloth"):void {
		if (armorType == "armor" || armorType == "robes") { //Armor or robes
	...
	*/
	//Dragonscale Armour
	/*
	function craftDragonscaleArmor(): void {
		clearOutput();
		...
		if (player.hasItem(Items.Materials.DragonScale, 5)) {
			addButton(0, "Armor", craftDragonscaleArmorForReal, 0, null, null, Items.Armor.DragonscaleArmor.description);
			addButton(1, "Robe", craftDragonscaleArmorForReal, 1, null, null, Items.Armor.DragonscaleRobes.description);
		}
		...
		addButton(2, "Bra", craftDragonscaleArmorForReal, 2, null, null, Items.Undergarments.DragonscaleBra.description);
		addButton(3, "Thong", craftDragonscaleArmorForReal, 3, null, null, Items.Undergarments.DragonscaleThong.description);
		addButton(4, "Loincloth", craftDragonscaleArmorForReal, 4, null, null, Items.Undergarments.DragonscaleLoincloth.description);
		addButton(14, "Nevermind", returnToRathazulMenu);
	}
	function craftDragonscaleArmorForReal(type) {
	...
	*/
	//Marae's Bark Armour
	/*
	function craftMaraeArmor(divine) {
		clearOutput();
		if (!divine) {
	...
	*/
	...
	function buyDyes(): void {
		...
		menu();
		addButton(0, "Auburn", buyDye, Items.Consumables.HairDyeAuburn);
		addButton(1, "Black", buyDye, Items.Consumables.HairDyeBlack);
		addButton(2, "Blond", buyDye, Items.Consumables.HairDyeBlond);
		addButton(3, "Brown", buyDye, Items.Consumables.HairDyeBrown);
		addButton(4, "Red", buyDye, Items.Consumables.HairDyeRed);
		addButton(5, "White", buyDye, Items.Consumables.HairDyeWhite);
		addButton(6, "Gray", buyDye, Items.Consumables.HairDyeGray);
		if (gameFlags[RATHAZUL_PURCHASE_COUNTER] >= 8) {
			addButton(7, "Blue", buyDye, Items.Consumables.HairDyeBlue);
			addButton(8, "Green", buyDye, Items.Consumables.HairDyeGreen);
			addButton(9, "Orange", buyDye, Items.Consumables.HairDyeOrange);
			addButton(10, "Purple", buyDye, Items.Consumables.HairDyePurple);
			addButton(11, "Pink", buyDye, Items.Consumables.HairDyePink);
		}
		if (gameFlags[RATHAZUL_PURCHASE_COUNTER] >= 12) {
			addButton(12, "Rainbow", buyDye, Items.Consumables.HairDyeRainbow);
		}
		addButton(14, "Nevermind", buyDyeNevermind);
	}
	function buyDye(dye:Item):void {
		clearOutput();
	...
	//Skin Oils
	function buyOils(): void {
		clearOutput();
		outputText("Rathazul smiles and pulls forth several bottles of skin oil. Which type of skin oil would you like?");
		outputText("<br><br><b>(-50 Gems)</b>");
		player.changeGems(-50);
		menu();
		addButton(0, "Dark", buyOil, Items.Consumables.SkinOilDark);
		addButton(1, "Ebony", buyOil, Items.Consumables.SkinOilEbony);
		addButton(2, "Fair", buyOil, Items.Consumables.SkinOilFair);
		addButton(3, "Light", buyOil, Items.Consumables.SkinOilLight);
		addButton(4, "Mahogany", buyOil, Items.Consumables.SkinOilMahogany);
		addButton(5, "Olive", buyOil, Items.Consumables.SkinOilOlive);
		addButton(6, "Russet", buyOil, Items.Consumables.SkinOilRusset);
		addButton(14, "Nevermind", buyOilNevermind);
	}
	function buyOil(oil:Item):void {
		clearOutput();
		//outputText(images.showImage("rathazul-buy-oil"));
		gameFlags[RATHAZUL_PURCHASE_COUNTER]++;
		Inventory.takeItem(oil, returnToRathazulMenu);
	}
	function buyOilNevermind(): void {
		clearOutput();
	...
	//Body Lotions
	function buyLotions(): void {
		clearOutput();
		outputText("Rathazul smiles and pulls forth several vials of body lotion. Which type of body lotion would you like?");
		outputText("<br><br><b>(-50 Gems)</b>");
		player.changeGems(-50);
		menu();
		addButton(0, "Clear", buyLotion, Items.Consumables.BodyLotionClear);
		addButton(1, "Rough", buyLotion, Items.Consumables.BodyLotionRough);
		addButton(2, "Sexy", buyLotion, Items.Consumables.BodyLotionSexy);
		addButton(3, "Smooth", buyLotion, Items.Consumables.BodyLotionSmooth);
		addButton(14, "Nevermind", buyLotionNevermind);
	}
	function buyLotion(lotion:Item):void {
		clearOutput();
		//outputText(images.showImage("rathazul-buy-lotion"));
		gameFlags[RATHAZUL_PURCHASE_COUNTER]++;
		Inventory.takeItem(lotion, returnToRathazulMenu);
	}
	function buyLotionNevermind(): void {
		clearOutput();
	...
	function purifySomething(): void {
		...
		//Item purification offer
		if (player.hasItem(Items.Consumables.IncubiDraft)) {
			addButton(0, "Incubi Draft", purifyItem, Items.Consumables.IncubiDraft);
		}
		if (player.hasItem(Items.Consumables.SuccubiMilk)) {
			addButton(1, "Succubi Milk", purifyItem, Items.Consumables.SuccubiMilk);
		}
		if (player.hasItem(Items.Consumables.SuccubiDelight)) {
			addButton(2, "S. Delight", purifyItem, Items.Consumables.SuccubiDelight);
		}
		/*if (player.hasItem(Items.Consumables.LaBova)) {
		 addButton(3, "LaBova", purifyItem, Items.Consumables.LaBova);
		 }
		 if (player.hasItem(Items.Consumables.MinotaurCum)) {
		 addButton(4, "Minotaur Cum", purifyItem, Items.Consumables.MinotaurCum);
		 }*/
		addButton(14, "Back", rathazulWorkOffer);
	}

	//PURIFICATION
	function purifyItem(item:Item):void {
		clearOutput();
		if (player.gems < 20) {
	...
	/*
	function purificationByRathazulBegin(): void {
	...
	function rathazulMakesPurifyPotion(): void {
		clearOutput();
		...
		menu();
		addButton(0, "Next", returnToRathazulMenu);
	}
	*/
	//Debimbo Draft
	/*
