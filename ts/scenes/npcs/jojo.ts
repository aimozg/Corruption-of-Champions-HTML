...
namespace JojoScene {
	addToGameFlags(JOJO_MET, JOJO_CAMP, JOJO_CORRUPTION_STAGE, JOJO_RAPE_COUNTER, JOJO_MEDITATION_COUNTER,
	...
	// SCENES
	//------------
	export function routeJojoEncounter(): void {
		//If Jojo is not encounterable.
		if (gameFlags[JOJO_CAMP] > 0/* || gameFlags[JOJO_DEAD_OR_GONE] > 0*/) {
	...
	function lowCorruptionJojoEncounter(): void {
		...
		if (player.tou >= 50 && player.isBiped())
			outputText(" You take a few steps back, momentarily dazed. Shaking it off, you ready your " + player.weaponName + " and assume a fighting stance.<br><br>");
			...
			outputText("The blow does little more than leave you momentarily dazed but isn’t enough to knock you over. You shake it off and ready your " + player.weaponName + " as you assume a fighting stance.<br><br>");
	...
	function repeatJojoEncounter(): void {
		...
		if (player.hasStatusEffect(StatusEffects.Infested)) {
			...
			addButton(1, "Purge", wormRemoval, null, null, null, "Request him to purge the worms from your body.");
			...
			doYesNo(meditateInForest, Camp.returnToCampUseOneHour);
		}
	...
	function meditateInForest(): void {
		...
		if (gameFlags[JOJO_MEDITATION_COUNTER] >= 5) {
			...
			/*if (gameFlags[FUCK_FLOWER_LEVEL] >= 4 && gameFlags[FUCK_FLOWER_KILLED] == 0 && gameFlags[JOJO_MEDITATION_COUNTER] % 5 == 0) {
		...
		if (gameFlags[JOJO_MEDITATION_COUNTER] % 5 == 0) {
			...
			doYesNo(acceptJojoIntoYourCamp, Camp.returnToCampUseTwoHours);
			return;
		}
		else
	...
	export function jojoCamp(): void {
		clearOutput();
		jojoSprite();
		/*if (gameFlags[AMILY_MET_PURE_JOJO] == 0 && gameFlags[AMILY_FOLLOWER] == 1 && amilyScene.amilyFollower()) {
		 getGame().followerInteractions.amilyMeetsPureJojo();
		 return;
		 }
		 if (gameFlags[JOJO_RATHAZUL_INTERACTION_COUNTER] == 1 && rand(2) == 0) {
		...
		if (player.cor > 10 /*&& gameFlags[JOJO_LAST_MEDITATION] != time.days*/) { //New "offer of help" menu
			if (player.cor >= 40)
			...
			/*if (player.lust >= 33 && player.gender > 0 && gameFlags[JOJO_CORRUPTION_STAGE] >= 0) addButton(4, "Rape", jojoAtCampRape);*/
		} else { //Normal shit
			if (player.cor > 10)
				...
				switch (rand(3)) {
					...
					case 1:
						...
						break;
					default:
						outputText("You find Jojo sitting cross-legged on a flat rock with his staff leaning against his shoulder, thinking. He looks to you and nods, \"<i>Greetings, " + player.name + ". Is there something I could do to assist you?</i>\"<br><br>");
				}
			}
			jojoCampMenu();
		}
	...
	function jojoCampMenu(): void {
		...
		if (gameFlags[JOJO_TRAINING_UNLOCKED] > 0) addButton(2, "Train", apparantlyJojoDOESlift, null, null, null, "Join him in a training session.");
		addButton(3, "Meditate", jojoFollowerMeditate);
		addButton(4, "N.Watch: " + (gameFlags[JOJO_NIGHT_WATCH] > 0 ? "On" : "Off"), jojoDefenseToggle, null, null, null, (gameFlags[JOJO_NIGHT_WATCH] > 0 ? "Request him to stop guarding the camp." : "Request him to guard the camp at night."));
		...
		/*if (player.cor > 10 && player.lust >= 33 && player.gender > 0 && gameFlags[JOJO_RAPE_DISABLED] <= 0) addButton(8, "Rape", jojoAtCampRape, null, null, null, "Rape the poor monk mouse-morph." + (player.cor < 40 ? " Why would you do that?" : ""));*/
	...
	function talkMenu(): void {
		...
		if (gameFlags[JOJO_TIMES_TALKED_WITH] >= 4) addButton(4, "You", jojoTalkYourOrigin, null, null, null, "Tell him about Ingnam and your history.");
		if (gameFlags[FACTORY_SHUTDOWN] > 0) addButton(5, "Factory", jojoTalkFactory, null, null, null, "Tell him about how you've shut down the factory.");
		/*if (gameFlags[SAND_WITCHES_COWED] == 1 || gameFlags[SAND_WITCHES_FRIENDLY] == 1 || gameFlags[SAND_MOTHER_DEFEATED] == 1) addButton(6, "SandCave", jojoTalkSandCave, null, null, null, "Tell him about your encounter in the Sand Cave in the desert.");*/
		if (gameFlags[JOJO_TRAINING_UNLOCKED] == 0 && gameFlags[JOJO_TIMES_TALKED_WITH] >= 4) addButton(7, "Training", apparantlyJojoDOESlift, null, null, null, "Ask him if he's willing to train you.");
		//if (gameFlags[MINERVA_PURIFICATION_JOJO_TALKED] == 1 && gameFlags[MINERVA_PURIFICATION_PROGRESS] < 10) addButton(8, "Purification", minervaScene.minervaPurification.purificationByJojoPart1, null, null, null, "Ask him if he can exorcise the demonic parasite infesting Minerva.");
		//Sex button
		if (player.cor <= 10 && player.lust >= 33) {
			...
			if (gameFlags[JOJO_TIMES_TALKED_WITH] < 4) addButtonDisabled(9, "Sex?", "You should socialize with Jojo a bit more.");
		...
		if (player.cor <= 10 && player.lust >= 33 && gameFlags[JOJO_CORRUPTION_STAGE] == -1) addButtonDisabled(9, "Sex?", "You need to spend more time with Jojo. <br><br>Talk sessions: " + gameFlags[JOJO_TIMES_TALKED_WITH] + "/6 <br>Training sessions: " + gameFlags[JOJO_TIMES_TRAINED_WITH] + "/10 <br>Meditation sessions: " + gameFlags[JOJO_MEDITATION_COUNTER] + "/10 <br>You must be pure enough and have sufficient lust as well.");
		if (player.cor <= 10 && player.lust >= 33 && gameFlags[JOJO_TIMES_TALKED_WITH] >= 6 && gameFlags[JOJO_TIMES_TRAINED_WITH] >= 10 && gameFlags[JOJO_TIMES_TALKED_WITH] >= 10 && gameFlags[JOJO_CORRUPTION_STAGE] > -3) addButton(9, "Sex?", offerSexFirstTimeHighAffection, null, null, null, "You've spent quite the time with Jojo, maybe you can offer him if he's willing to have sex with you?"); //Will unlock consensual sex scenes.
	...
	function jojoTalkVillage(): void {
		clearOutput();
		gameFlags[JOJO_TIMES_TALKED_WITH]++;
	...
	function jojoTalkFactory(): void {
		...
		if (gameFlags[FACTORY_SHUTDOWN] == 2) {
	...
	/*function jojoTalkSandCave():void {
	 ...
	 if (gameFlags[SAND_WITCHES_COWED] == 1) {
	 ...
	 else if (gameFlags[SAND_WITCHES_FRIENDLY] == 1 && gameFlags[SAND_MOTHER_DEFEATED] == 1) {
	 ...
	 else if (gameFlags[SAND_WITCHES_FRIENDLY] == 1 && gameFlags[SAND_MOTHER_DEFEATED] == 0) {
	 ...
	 if (gameFlags[MILK_NAME] == undefined) {
	 ...
	 outputText("As the question leaves his lips you give Jojo a confused look and, with a glance, direct his gaze toward " + gameFlags[MILK_NAME] + ".<br><br>");
	 outputText("He slaps his own forehead and says, “<i>Oh... yeah... right.</i>” Obviously embarrassed by not putting two and two together. He smiles good naturedly though, “<i>I don’t know I guess I just assumed you found some poor mind addled soul and decided to save her.</i>” Jojo says as he looks over at " + gameFlags[MILK_NAME] + ".<br><br>");
	 if (gameFlags[MILK_SIZE] == 0) { //[if (bathSlutStage1 - unaltered)
	 outputText("“<i>She’ll fare much better in our care than in the coven’s,</i>” he states with conviction.<br><br>");
	 }
	 else if (gameFlags[MILK_SIZE] == 1) { //[if (bathSlutStage2 - HHH)
	 ...
	 //[if {PC met Cum Witch}
	 if (gameFlags[CUM_WITCH_DEFEATED] == 1 || gameFlags[BEEN_BLESSED_BY_CUM_WITCH] == 1) {
	 ...
	 if (gameFlags[MORE_CUM_WITCHES] == 1) {
	 ...
	 else if (gameFlags[CUM_WITCHES_FIGHTABLE] == 1) {
	...
	function apparantlyJojoDOESlift(): void {
		clearOutput();
		//{First Session only}
		if (gameFlags[JOJO_TRAINING_UNLOCKED] == 0) {
			gameFlags[JOJO_TRAINING_UNLOCKED] = 1;
		...
		gameFlags[JOJO_TIMES_TRAINED_WITH]++;
		...
		if (!player.hasPerk(PerkLib.ControlledBreath) && !player.hasPerk(PerkLib.CleansingPalm) && !player.hasPerk(PerkLib.Enlightened)) {
			...
			if (gameFlags[JOJO_TIMES_TRAINED_WITH] == 5) {
		...
		//{after the PC has gained the controlled breath perk}
			...
			player.changeFatigue(60, true);
			if (gameFlags[JOJO_TIMES_TRAINED_WITH] == 10) {
		...
		//{after the PC has gained the Cleansing Palm attack}
			...
			player.changeFatigue(60, true);
			if (gameFlags[JOJO_TIMES_TRAINED_WITH] >= 16 && player.inte >= 70) {
		...
		//{after PC has gained the Enlightened Perk}
			...
			const enlightenedBlurbs = [] as string[];
			...
			//if (gameFlags[AMILY_FOLLOWER] > 0) enlightenedBlurbs.push("You can hear Amily changing the bedding to her nest.");
			...
			//if (gameFlags[UNKNOWN_FLAG_NUMBER_00238] > 0) enlightenedBlurbs.push("You can hear Izma flipping through the pages of a book."); // TODO: (if Izmael gets put in) you can hear Izmael doing push ups to stay fit.
		...
		//Boost attributes!
		if (player.str < 50) player.dynStats("str", 0.5);
		if (player.str < 80) player.dynStats("str", 0.5);
		if (player.inte < 50) player.dynStats("inte", 0.5);
		if (player.inte < 80) player.dynStats("inte", 0.5);
		menu();
		doNext(Camp.returnToCampUseOneHour);
	}
	...
	function jojoFollowerMeditate(doClear: boolean = false): void {
		...
		if (gameFlags[JOJO_LAST_MEDITATION] == time.days) {
			...
			if (player.cor > 80)
				...
				cleanse -= 1;
			player.dynStats("cor", cleanse - player.countCockSocks("alabaster"));
			...
			gameFlags[JOJO_LAST_MEDITATION] = time.days;
			gameFlags[JOJO_MEDITATION_COUNTER]++;
		}
		doNext(Camp.returnToCampUseOneHour);
	}
	...
	function confrontChastity(): void {
		...
		gameFlags[JOJO_RAPE_DISABLED]    = 1;
	...
	function pureJojoSexMenu(): void {
		//Capacity
		let capacity = 40;
		if (gameFlags[JOJO_ANAL_XP] < 10) capacity += (gameFlags[JOJO_ANAL_XP] * 3);
		...
		if (gameFlags[JOJO_SEX_COUNTER] < 3) outputText("Jojo looks into your eyes and says, \"<i>Only if you're going to be gentle.</i>\"");
	...
	function anallyFuckTheMouseButtSlut(): void {
		//Capacity
		let capacity = 40;
		if (gameFlags[JOJO_ANAL_XP] < 10)
			capacity += (gameFlags[JOJO_ANAL_XP] * 3);
		else
			capacity += 30; //Caps at 70.
		let x = player.cockThatFits(capacity);
		//Begin
		clearOutput();
		if (gameFlags[JOJO_ANAL_XP] == 0) outputText("You finally make up your mind; you want to stuff your cock into that tight ass of his. " + player.clothedOrNaked("You remove your [armor] and ") + "Jojo hesitantly strips out of his robe, revealing his naked form.");
		else if (gameFlags[JOJO_ANAL_XP] == 1) outputText("You decide that you want to stuff your cock into that tight ass of his again. " + player.clothedOrNaked("You remove your [armor] and ") + "Jojo hesitantly strips out of his robe, revealing his naked form.");
		else if (gameFlags[JOJO_ANAL_XP] >= 2) outputText("You decide that you want to stuff your cock into that tight ass of his again. " + player.clothedOrNaked("You remove your [armor] and ") + "Jojo without hesitation strips out of his robe, revealing his naked form.");
		//First and second time
		if (gameFlags[JOJO_ANAL_XP] < 3) {
			//Intro
			if (gameFlags[JOJO_ANAL_XP] < 1) outputText("<br><br>\"<i>Just go gentle,</i>\" Jojo says with a whimpered look on his face as he gets down on all fours.");
			...
			if (gameFlags[JOJO_ANAL_XP] < 1) outputText("You stretch out Jojo’s bowels as much as you can and it should be just enough for insertion. ");
			...
			outputText("<br><br>You spit on your hands and apply saliva evenly all over your " + player.cockDescript(x) + " to get it all lubed up. Deeming the lubrication sufficient, you let Jojo know that you’re going to start and you slowly slide your " + player.cockDescript(x) + " into Jojo’s butthole. " + (gameFlags[JOJO_ANAL_XP] > 0 ? "His sphincter closes around you tightly. By Marae, he’s tight as ever!" : "By Marae, he's tight!") + " ");
			if (gameFlags[JOJO_ANAL_XP] < 1) {
		...
		//The End
		if (gameFlags[JOJO_SEX_COUNTER] >= 4) outputText("He plants a kiss on your lips.");
		...
		gameFlags[JOJO_ANAL_XP]++;
		gameFlags[JOJO_SEX_COUNTER]++;
		player.orgasm();
		doNext(Camp.returnToCampUseOneHour);
	}
	...
	function getAnallyFuckedByMouseYouSlut(): void {
		...
		clearOutput();
		outputText("You finally make up your mind; you want his mouse-cock in your [ass]. " + player.clothedOrNaked("You remove your [armor] and ") + "Jojo " + (gameFlags[JOJO_SEX_COUNTER] < 4 ? "hesitantly " : "") + "strips out of his robe, revealing his naked form.");
		...
		outputText("<br><br>His cock slides out of your [ass] with a pop. Cum start dripping out of your [ass]. \"<i>That was... You were amazing, [name].</i>\" You smile at him while rubbing your inflated belly and tell him he wasn’t too bad himself. " + (gameFlags[JOJO_SEX_COUNTER] >= 4 ? "You give Jojo a lingering kiss." : ""));
		outputText("<br><br>\After a good while of rest, " + player.clothedOrNaked("the two of you get redressed and", "Jojo gets redressed and the two of you") + " return to your camp.");
		player.dynStats(["sens", 1], ["cor", -1]);
		gameFlags[JOJO_ANAL_CATCH_COUNTER]++;
		gameFlags[JOJO_SEX_COUNTER]++;
		player.orgasm();
		player.slimeFeed();
		doNext(Camp.returnToCampUseOneHour);
	...
	function getVagFuckedByMouse(): void {
		...
		clearOutput();
		outputText("You finally make up your mind; you want his mouse-cock in your [vagina]. " + player.clothedOrNaked("You remove your [armor] and ") + "and Jojo " + (gameFlags[JOJO_SEX_COUNTER] < 4 ? "hesitantly " : "") + " strips out of his robe, revealing his naked form.");
		if (gameFlags[JOJO_VAGINAL_CATCH_COUNTER] == 0) {
		...
		else if (gameFlags[JOJO_VAGINAL_CATCH_COUNTER] == 1) {
		...
		else if (gameFlags[JOJO_VAGINAL_CATCH_COUNTER] >= 2) {
		...
		player.dynStats(["sens", 1], ["cor", -1]);
		gameFlags[JOJO_VAGINAL_CATCH_COUNTER]++;
		gameFlags[JOJO_SEX_COUNTER]++;
	...
	function giveBirthToPureJojoBabies(): void {
		...
		if (gameFlags[AMILY_FOLLOWER] == 1) {
			if (gameFlags[JOJO_LITTERS_AMILY_REACTION_COUNTER] == 0) {
			...
			gameFlags[JOJO_LITTERS_AMILY_REACTION_COUNTER]++;
		}
	...
	function suckJojosCock(): void {
		...
		outputText("<br><br>Instead of resisting him, you start moving in time with him, deep-throating his tasty cock. Jojo start moaning louder and started rocking his hips back and forth rapidly. It felt like he was raping your throat. Jojo didn’t seem to be cumming anytime soon and you were getting impatient. You suddenly jab your finger into Jojo’s ass and stimulate his prostate. Panting, Jojo shouts, \"<i>I’m gonna cum!</i>\" and pull your head forward as much as possible. Hot cum came gushing out like a hose, instantly " + (jojoCumQ() >= 600 ? "distending" : "stuffing") + " your stomach. Jojo falls back, his cock spraying cum all over your [face] and running down your [fullchest]. " + (jojoCumQ() >= 900 ? "His orgasm seems to never end and he continues to cum for a good while." : ""));
		if (gameFlags[JOJO_BLOWJOB_XP] <= 0)
	...
	function jojosThirdRape(): void {
		...
		if (player.gender == 2) { //Females
			...
			if (player.hasLongTail()) outputText("Your tail swishes back and forth teasingly. ");
	...
	function corruptJojoSexMenu() {
		...
		if (player.hasCock()) {
			...
			addButton(8, "Cruel Anal", corruptJojoAnalCruel); //Overrides Anal Smother - Herms don't smother, they fuck
		}
	...
	function corruptJojoCunnilingus(): void {
		...
		switch (rand(3)) {
			case 0:
				...
				break;
			case 1:
				outputText("You take the tip of his member into your mouth and slowly start to bob your head, one hand squeezing at his balls tenderly as your other hand strokes the length of his cock that your lips don't reach. You let your pace quicken over time, mimicking a vigorous fucking.  ");
				break;
			default:
