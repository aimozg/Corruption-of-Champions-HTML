namespace LumiScene {
	...
	export function lumiEncounter(): void {
		...
		if (gameFlags[LUMI_MET] == 0) {
			...
			outputText("You spot an anomaly in the barren wastes; a door that seems to be standing out in the middle of nowhere. Somehow, you figure that it must lead to some other part of the world, and the only reason it's here is because you can't get to where the door should be right now.<br><br>");
	...
	function lumiLabChoices(): void {
		...
		if (gameFlags[LUMI_MET] == 0) {
			...
			outputText("\"<i>Stho, what can Lumi, Gobin Aochomist Extwaordinaire, do fo you today?</i>\" asks the unusual goblin.<br><br>");
	...
	function lumiShop(): void {
		...
		menu();
		addButton(0, Items.Consumables.LustDraft.shortName, lumiLustDraftPitch);
		addButton(1, Items.Consumables.GoblinAle.shortName, lumiPitchGobboAle);
		addButton(2, Items.Consumables.OviElixir.shortName, lumiPitchOviElixer);
		addButton(4, "Leave", lumiLabChoices);
	}
	...
	function lumiLustDraftPitch(): void {
		...
		doYesNo(createCallBackFunction(lumiPurchase, Items.Consumables.LustDraft), lumiShop);
	}
	...
	function lumiPitchGobboAle(): void {
		...
		doYesNo(createCallBackFunction(lumiPurchase, Items.Consumables.GoblinAle), lumiShop);
	}
	...
	function lumiPitchOviElixer(): void {
		...
		doYesNo(createCallBackFunction(lumiPurchase, Items.Consumables.OviElixir), lumiShop);
	}


	function lumiPurchase(itype: Item): void {
		clearOutput();
		...
		if (itype == Items.Consumables.LustDraft)
			cost = 15;
		if (player.gems >= cost) {
			outputText("You pay Lumi the gems, and she hands you " + itype.longName + " saying, \"<i>Here ya go!</i>\"<br><br>");
			player.changeGems(-cost);
			Inventory.takeItem(itype, lumiShop, lumiLabChoices);
		}
		else {
			//After choosing, and PC doesn't have enough gems
			outputText("You go to pay Lumi the gems, but then you realize that you don't have enough. Lumi seems to know what happened and tells you \"<i>Ok, is dere somefing you want to buy that you can affowd?</i>\"<br><br>");
	...
	/*LumiScene.lumiEnhance = function (justCheck) {
	 ...
	 clearOutput();
	 outputText("\"<i>Do you have 100 gems for de enhancement?</i>\" asks Lumi.<br><br>");
	 //If (player has less than 100 gems)
	 if (player.gems < 100) {
	 outputText("You shake your head no, and Lumi gives you a disappointed look and says, \"<i>Den Lumi can do no enhancement for you. Anyfing else?</i>\"<br><br>");
	 //Return to main Lumi menu
	 doNext(lumiLabChoices);
	 return false;
	 }
	 else {
	 outputText("You nod and Lumi gives an excited yell, \"<i>Yay! Lumi loves to do enhancement, what you want to be bettar?</i>\"<br><br>");
	 ...
	 temp = rand(3);
	 if (itype == consumables.GLDSEED) outputText("She fiddles with it, coating it in exotic powders before she tosses the whole mess onto a hotplate. It explodes, knocking the goblin flat on her ass. She sits bolt upright and snatches up the now-glowing seed with a gloved hand.<br><br>");
	 ...
	 else if (itype == consumables.KANGAFT) outputText("She fiddles with it, coating it in exotic powders before she tosses the whole mess onto a hotplate. It explodes, knocking the goblin flat on her ass. She sits bolt upright and snatches up the now-glowing fruit with a gloved hand.<br><br>");
	 else if (temp == 0) outputText("She starts grabbing things from around the table, seemingly at random, and adds them to " + itype.longName + ". To your alarm, there is soon a large cloud of smoke coming off it! There is a strong smell to the smoke and it makes it hard to breathe. Lumi grabs a mask out of a drawer and puts it on, continuing with her work unperturbed. She suddenly stops and you wonder if she is done, but she takes off her mask and inhales deeply of the smoke, then keels over! As you go over to help her she suddenly stands up, waves away some of the smoke, and says, \"<i>All dun!</i>\"<br><br>");
