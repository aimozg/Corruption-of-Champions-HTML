class Imp extends Monster {
	constructor() {
		...
		this.gems       = 5 + rand(5);
		//Battle variables
		this.weapon     = Items.monsterWeapon("claws", 0, "claw-slash");
		this.armor      = Items.monsterArmor("leathery skin", 0);
		this.lustVuln   = 1;
...
// Initial Win Scenes
	...
	export function victoryAgainstImp(): void {
		clearOutput();
		//Screen messages
		let canFeed       = (player.hasStatusEffect(StatusEffects.Feeder));
		let canBikiniTits = (player.hasVagina() && player.biggestTitSize() >= 4 && player.armor == Items.Armor.LustyMaidenArmor);
		...
		if (player.lust >= 33) {
			if (player.hasCock()) {
				if (player.cockThatFits(monster.analCapacity()) == -1)
					...
					addButton(0, (player.isTaur() ? "Centaur Rape" : "Male Rape"), (player.isTaur() ? centaurOnImpStart : rapeImpWithDick));
	...
	export function rapeImpWithDick(condomed: boolean = false): void {
		clearOutput();
		let x = player.cockThatFits(monster.analCapacity());
		if (x < 0) x = 0;
		if (condomed) {
		...
		player.orgasm();
		if (!condomed) player.dynStats("cor", 1);
		cleanupAfterCombat();
	}
	
	// FEMALE RAPE
	
	export function rapeImpWithPussy(): void {
		clearOutput();
		...
		if (player.vaginalCapacity() < monster.cockArea(0)) {
			...
			//Preggers chance!
			player.knockUp(PregnancyType.IMP, INCUBATION_IMP);
	...
	export function centaurOnImpStart() {
		clearOutput();
		...
		if (x >= 0 && !player.hasVagina()) centaurOnImpMale();
			...
			menu();
			addButton(0, "Male", ImpScene.centaurOnImpMale, false, null, null, "TO BE ADDED.");
			addButton(1, "Female", ImpScene.centaurOnImpFemale, false, null, null, "TO BE ADDED.");
		}
	}
	
	// MALE RAPE AS TAUR
	
	export function centaurOnImpMale(vape: boolean = false): void {
		let x = player.cockThatFits(monster.analCapacity());
		if (x < 0) x = 0;
		if (vape) clearOutput();
		// Strange cocks first.
	...
	export function centaurOnImpFemale(vape: boolean = false): void {
		if (vape) clearOutput();
	...
	export function centaurGirlOnImps(): void {
		clearOutput();
	...
	export function centaurOnImpResults(iNum: number): void {
		let x = player.cockThatFits(monster.analCapacity());
		if (x < 0) x = 0;
	...
	export function impNippleRape(): void {
		clearOutput();
		...
		player.orgasm();
		player.dynStats(["sens", -3], ["cor", 1]);
		cleanupAfterCombat();
	}
	
	// FEEDER SCENE WITH IMP
	
	export function areImpsLactoseIntolerant(): void {
		clearOutput();
	...
	export function putBeeEggsInAnImp(): void {
		clearOutput();
		//IMP EGGS
	...
	export function killImp(): void {
		...
		if (player.cor < 25) player.dynStats("cor", -0.5);
		menu();
	...
	export function takeSkull(): void {
	...
	export function impRapesYou(): void {
		...
		if ((player.hasPerk(PerkLib.BimboBrains) || player.hasPerk(PerkLib.FutaFaculties)) && !player.isTaur() && player.hasVagina()) {
			...
			player.knockUp(PregnancyType.IMP, INCUBATION_IMP - 14); //Bigger imp means faster pregnancy
			
			player.orgasm();
			player.dynStats(["lib", 1], ["sens", 1], ["cor", 1]);
			cleanupAfterCombat();
			return;
		}
		//Lust loss
		if (player.lust >= player.maxLust()) {
			...
			if (player.gender == 2 || player.gender == 3) {
				...
				outputText("<br><br>He smiles and presses his cock against your " + player.vaginaDescript(0) + ".  Your lust-driven mind is speechless, leaving you panting and moaning like a whore.");
				...
				player.knockUp(PregnancyType.IMP, INCUBATION_IMP - 14); //Bigger imp means faster pregnancy
				player.dynStats(["lib", 1], ["sens", 1], ["lust", 1], ["cor", 1]);
			...
			if (player.gender == 0 || player.gender == 1) {
				...
				if (player.gender == 1 && rand(2) == 0) {
					...
					outputText("<br><br>You drop to the ground when he's done with you, cum spilling from your abused ass all over the ground, too exhausted to move.  Consciousness fades.  ");
				}
			}
			cleanupAfterCombat();
			player.orgasm();
	...
	export function sprocketImp(): void {
		player.slimeFeed();
		clearOutput();
		...
		//Variable cocktext
		if (player.cocks[0].cockType == CockTypesEnum.HUMAN || player.cocks[0].cockType == CockTypesEnum.DEMON) outputText("your " + player.cockDescript(0) + ", rubbing the tip of his prick against your own, ");
		...
		if (player.cocks[0].cockType == CockTypesEnum.HUMAN || player.cocks[0].cockType == CockTypesEnum.DEMON) outputText("shoves his shaft deeply into the slit in the head of your dick.  ");
