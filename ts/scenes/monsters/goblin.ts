// Goblins originally created by ???.
// Updated for CoC HTML mod by Mattibun.

// Original notes for goblins:
...
namespace GoblinScene {
	...
	export function victoryAgainstGoblin(): void {
		clearOutput();
		if (monster.HP <= 0)
		...
		if (player.lust < 30 && !player.hasStatusEffect(StatusEffects.Feeder)) { // Check this Status Effect check
			cleanupAfterCombat();
			return;
		}
		...
		if (player.lust >= 33 && player.gender > 0) {
			...
			if (player.hasCock()) {
				if (player.cockThatFits(monster.vaginalCapacity()) >= 0) {
					addButton(0, "Dick Fuck", gatsGoblinBoners, null, null, null, "TO BE ADDED");
					addButton(4, "Jog Fuck", gobboGetsRapedMaleFits, null, null, null, "TO BE ADDED");
				}
				if (player.cockArea(player.biggestCockIndex()) > monster.vaginalCapacity()) {
					addButton(1, "DickTooBig", manRapesGoblinTooBig, null, null, null, "TO BE ADDED");
				}
				if (player.cockArea(player.biggestCockIndex()) > monster.vaginalCapacity() && player.cor > 80) { // && kGAMECLASS.monk > 2) FIGURE OUT WHAT THIS IS?
					addButton(2, "CorruptBig", rapeAGoblinCorruptTooBig, null, null, null, "TO BE ADDED");
				}
				if (player.cockThatFits(monster.analCapacity()) >= 0 && (player.cor > 70 - player.corruptionTolerance() || gameFlags[MEANINGLESS_CORRUPTION] >= 1)) {
					addButton(3, "Dick In Ass", goblinButtSex, null, null, null, "TO BE ADDED");
				}
				if (player.hasStatusEffect(StatusEffects.Feeder)) { //CHECK THIS STATUS EFFECT!
					addButton(5, "Breastfeed", giveGoblinAMilkMustache, null, null, null, "TO BE ADDED");
				}
				if (player.tailType == TailType.SPIDER_ADBOMEN && player.cockThatFits(monster.vaginalCapacity()) >= 0) {
					addButton(6, "SpiderCondom", goblinCondomed, "web", null, null, "TO BE ADDED"); // Pass argument to do Spider condom
				}
				else if (player.hasItem(Items.Consumables.Condom) && player.cockThatFits(monster.vaginalCapacity()) >= 0) {
					addButton(6, "Use Condom", goblinCondomed, "latex", null, null, "TO BE ADDED"); // Pass argument to do normal condom
				}
				
			}
			if (player.hasVagina()) {
				addButton(7, "Pussies", goblinFemaleRape, null, null, null, "TO BE ADDED");
			}
			if (player.canOvipositSpider()) {
				addButton(8, "Lay Eggs", goblinDriderEggs, null, null, null, "TO BE ADDED");
			}
		}
		
		// Not horny enough, but can feed or oviposit
		if (player.canOvipositSpider() || player.hasStatusEffect(StatusEffects.Feeder)) { // CHECK STATUS EFFECT
			outputText("<br><br><b>You aren't horny enough to rape her, but ");
			if (player.hasStatusEffect(StatusEffects.Feeder)) outputText("your nipples ache with the desire to feed her your milk.  Do you feed her milk or leave?</b>");
	...
	// Standard male fuck
	export function gatsGoblinBoners(): void {
		clearOutput();
	...
	// Male jog fuck
	export function gobboGetsRapedMaleFits(): void {
	...
	export function goblinFemaleRape(): void {
		//spriteSelect(24);
		clearOutput();
		// Are you a taur?
		if (player.isTaur()) {
	...
	// Dick too big
	export function manRapesGoblinTooBig(): void {
		clearOutput();
	...
	export function rapeAGoblinCorruptTooBig(): void {
		var x = player.biggestCockIndex();
		clearOutput();
		//spriteSelect(24);
		//outputText(images.showImage("goblin-win-male-corruptedfuck"));
		outputText("You begin to remove your " + player.armor.equipmentName + ", looking down on your helpless would-be-attacker and soon-to-be victim while licking your lips hungrily. Your " + player.multiCockDescriptLight());
	...
	export function goblinButtSex(): void {
		//spriteSelect(24);
		var x = player.cockThatFits(monster.analCapacity());
		if (x < 0) x = 0;
		clearOutput();
		...
		outputText("As you pick up your " + player.armor.equipmentName + " and begin to get dressed, you glance at the goblin. Her hands began to dig in her now stretched out anus, desperately trying to gather up the cum you deposited in her. Smirking, you walk away nonchalantly, quite pleased with yourself.");
		cleanupAfterCombat();
		player.orgasm();
	}
	
	// Breastfeed Goblin if you're a feeder
	export function giveGoblinAMilkMustache(): void {
		clearOutput();
		...
		if (s) {
			s.value1 += 1;// CHECK THIS
			s.value2 = 0; // CHECK THIS
		}
		cleanupAfterCombat();
	}
	
	// Spider condom or regular condom
	export function goblinCondomed(mode: "latex" | "web"): void {
		//spriteSelect(24);
		...
		switch (mode) {
			case "web":  //Web condom (spinnerets)
	...
	export function defeatAgainstGoblin() {
		...
		else if (player.hasStatusEffect(StatusEffects.Infested)) {
	...
	export function goblinRapesPlayer() {
		...
		if (player.totalCocks() > 0 && (!player.hasVagina() || rand(2) == 0)) {
			...
			if (player.cockArea(0) > monster.vaginalCapacity()) {
				...
				outputText(" before she buries it deep into your " + Appearance.assholeOrPussy(player) + ".  Her thighs, breasts, and body surround you with a shroud of warm green flesh that wriggles and twists around you with feverish lust.  The lips of her still-partially stretched cunt do their best to devour your member's head when the flexible little minx curves her hips back down to grind on it.<br><br>");
...
class Goblin extends Monster {
	constructor() {
		...
		this.armor      = Items.monsterArmor("leather straps", 0);
		this.lustVuln   = 1;
	...
	public static goblinDrugAttack() {
		...
		//Drink blue pots
			...
			if (monster.HPRatio() < 1) {
				...
				monster.changeHP(80 * multiplier);
			}
			...
			if (monster.HP < monster.maxHP()) {
				...
				monster.changeHP((monster.maxHP() / 4) * multiplier);
