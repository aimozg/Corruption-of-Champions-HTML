namespace WhitneyScene {
	//[YES]
	function whitneyMilkerHookup(breast:boolean = true):void {
		displaySprite("whitney");
		clearOutput();
		outputText("Whitney takes the gear back to her farm after promising to have it working within the hour. She did leave you with a cryptic warning to \"<i>leave the milkings to the beasts, lest you become one</i>.</i>\"<br><br>You shrug and head back to check up on camp.");
		if (breast) {
	...
	//TALK
	export function talkWhitney():void {
		displaySprite("whitney");
		//[FIND WHITNEY TXT]
		clearOutput();
		//Centaur Hookups!
		if (!player.hasKeyItem(KeyItems.ToyFakeMare) && player.isTaur()) {
			centaurToysHoooooo();
			return;
		}
		...
		if (rand(4) == 0 && !player.hasKeyItem(KeyItems.BreastMilkerInstalled)) {
			if (player.hasKeyItem(KeyItems.BreastMilker)) {
		...
		if (rand(4) == 0 && !player.hasKeyItem(KeyItems.CockMilkerInstalled) && player.hasKeyItem(KeyItems.CockMilker)) {
			outputText("Before you can say much of anything, Whitney exclaims, \"<i>My stars! Is that one of them demon's milking machines?</i>\"<br><br>");
		...
		if (player.inte < 15) player.dynStats("inte", 1);
		if (player.inte < 20) player.dynStats("inte", 1);
		if (player.inte < 30) player.dynStats("inte", .5);
		if (player.inte < 40) player.dynStats("inte", .5);
	...
	function centaurToysHoooooo():void {
		...
		outputText("\"<i>See, the two of them left some rather personal belongings behind,</i>\" Whitney says, swinging the barn door open. Inside, lying in the middle of a stall are what looks like a tall, slender totem with a giant rubber horsecock sticking out of it and a fake mare standing on reinforced wooden legs with a glistening, horsey onahole between its thighs. Oh, my. \"<i>So, " + player.name + ", since you look to have the right build for 'em... they're yours.</i>\"<br><br>");
