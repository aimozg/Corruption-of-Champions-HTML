...
namespace OasisScene {
	...
	export function oasisEncounter():void {
		...
		clearOutput();
		outputText("You wander in the desert for what seems like hours, sweating profusely in the sweltering heat. Eventually you come across a small watering hole surrounded by scrappy trees and shrubs. It would be foolish not to take this opportunity to drink, freshen up and paddle your " + player.legs() + " in the cooling water, so you settle into what little shade you can find for a quick break.<br><br>");
		//Demons approach!
	...
	export function oasisSexing():void {
		...
		if (player.vaginas.length > 0) {
			...
			outputText("<br><br>");
			//Preggers chance!
			player.knockUp(PregnancyType.IMP, INCUBATION_IMP, 90); // TODO Check this. May be time to implement rest of pregnancy.
		}
		//Buttbutt buuuuuttt
		...
		player.dynStats(["tou", .5], ["sens", .5], ["cor", 4]);
		if (inCombat()) {
			cleanupAfterCombat();
			player.orgasm();
		}
	...
	export function oasisBadEnd():void {
		...
		outputText("Your arms are also tied up behind your back, resting uncomfortably in the hot desert sand. You can see the tribe is packing up to get ready to move on, and you struggle to sit up against your bindings.<br><br>");
		outputText("\"<i>I see that you are awake, slave,</i>\" the leader says after watching you get up, walking closer to you with a wicked smirk on his lips. He stares down at you with a look of satisfaction on his face. \"<i>Thought you would wake up in time to sneak off on us again, did you? Well, that won't be happening anymore.</i>\"<br><br>");
		outputText("\"<i>What the hell are you talking about? I'm no one's slave!</i>\" you yell in indignation, only to cry out in pain as the tribe leader's hand comes down and smacks you across the face. Your head jerks back from the force of the blow, successfully silencing you.<br><br>");
		...
		//[If herm]
		if (player.gender == 3) {
			if (player.isTaur()) outputText("\"<i>Everyone will have a great time playing with you,</i>\" the leader says, smirking wider still as he stares at your centaur body. He reaches out and gives your " + player.buttDescript() + " a hard smack, causing you to yelp and quickly stand to your feet. You stagger a bit as your legs strain against the shackles as you stand. Without warning, he sticks his fingers inside of your " + player.assholeDescript() + " and stretches it out as his free hand reaches between your legs to play with both your " + player.vaginaDescript(0) + " and " + player.cockDescript(0) + ". You gasp and moan in pleasure as your cock becomes erect in his hand. He laughs as he lets go and moves back in front of you. \"<i>Plus we could use you to carry our heavy loads like a pack animal. You have the perfect body for it.</i>\"<br><br>");
			else outputText("His smirk widens even more as he stares down at your uncovered " + player.cockDescript(0) + " laying flaccidly between your legs against the hot desert sand. You gasp in surprise as he suddenly reaches down and takes a firm hold of your " + player.cockDescript(0) + ". He strokes it and gives it a rough squeeze, making you whimper sensually as it becomes fully erect in his hand. He laughs as he lets it go, \"<i>Our girls will definitely be having fun with you, slave.</i>\"<br><br>");
		}
	...
	export function demonLose() {
		...
		if (player.gender == 0) {
			...
			if (gameFlags[INFESTED] == 1) {
				outputText("<br><br>The demons joke and smile, obviously unconcerned with your state.<br><br>");
			}
			if (player.cocks.length > 0) {
...
//("Misdirection"
		else if (player.hasPerk(PerkLib.Misdirection) && rand(100) < 15 && player.armor.equipmentName == "red, high-society bodysuit") {
...
//Determine if cat'ed
			...
			let temp = ((monster.str + monster.weapon.attack) * (player.damagePercent() / 100)); //Determine damage - str modified by enemy toughness!
			if (temp <= 0) {
				if (!monster.plural)
				...
				if (temp <= 5)
					...
					outputText(monster.a + monster.name + " stagger you with the force of " + monster.hisHer + " " + monster.weapon.verb + "s! ");
