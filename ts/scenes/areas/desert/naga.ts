class Naga extends Monster {
	...
	static nagaPoisonBiteAttack() {
		...
		(monster as Naga).venom -= 25;
		if ((monster as Naga).venom <= 0) {
	...
	static nagaTailWhip() {
		...
		} else if (player.hasPerk(PerkLib.Misdirection) && rand(100) < 10 && player.armor.equipmentName == "red, high-society bodysuit") {
...
namespace NagaScene {
	...
	// SCENES
	//------------
	export function nagaEncounter() {
		clearOutput();
		displaySprite("naga");
		if (player.isNaga()) {
			nagaEncounterSex(true);
		} else {
	...
	function nagaEncounterSex(prompt: boolean): void {
		if (prompt) {
			clearOutput();
			...
			menu();
			addButton(0, "Sex", nagaEncounterSex, false, null, null, "Embrace the Naga and proceed to have sex.");
			addButton(1, "FIGHT!", nagaEncounterRefusalFight, null, null, null, "Realize what the Naga is doing to you and angrily reject her. This will lead to a fight.");
			return;
		}
	...
	function nagaEncounterRefusalFight():void {
		clearOutput();
	...
	export function nagaWin():void {
		nagaRapeChoice();
	}

	export function nagaLoss():void {
	...
	function nagaRapeChoice():void {
		...
		if (player.lust >= 33 && gameFlags[SFW_MODE] <= 0) {
			...
			menu();
			if (player.gender == 0) addButton(0, "Yes", nagaVictoryGenderless, null, null, null, "TO BE ADDED.");
			if (player.gender == 1) addButton(0, "Yes", nagaVictoryMale, null, null, null, "TO BE ADDED.");
			if (player.gender == 2) addButton(0, "Yes", nagaVictoryFemale, null, null, null, "TO BE ADDED.");
			if (player.gender == 3) {
				addButton(0, "As Male", nagaVictoryMale, null, null, null, "TO BE ADDED.");
				addButton(1, "As Female", nagaVictoryFemale, null, null, null, "TO BE ADDED.");
			}
			if (player.lowerBody == LowerBodyType.GOO) {
				addButton(2, "Gooey Rape", gooNagaRape, null, null, null, "TO BE ADDED.");
			}
			if (eggs != null)
