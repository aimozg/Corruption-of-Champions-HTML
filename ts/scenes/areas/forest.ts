namespace Areas {
	export namespace Forest {
		export function explore(): void {
			...
			let select            = choice[rand(choice.length)];
			switch (select) {
				case 0: //25% chance of Tamani/Daughter's. Otherwise, goblin and imp encounters. Trying a tweak of the encounter rate...
					if (rand(100) <= 25 && gameFlags[TAMANI_TIME_OUT] == 0 && player.gender > 0 && gameFlags[TAMANI_BAD_ENDED] == 0 && (player.totalCocks() > 0 || !player.hasKeyItem(KeyItems.ToyDeluxeDildo))) {
				...
				case 2: //Tentacle Beasts (Not yet implemented)
					...
					if (player.hasKeyItem(KeyItems.BookDangerousPlants) && player.inte / 2 > rand(50)) {
