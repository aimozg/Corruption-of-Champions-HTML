...
namespace GreenSlimeScene {
	export function encounterSlime(): void {
		clearOutput();
	...
	export function greenSlimeWin(): void {
		...
		if (player.hasStatusEffect(StatusEffects.Feeder) && gameFlags[SFW_MODE] <= 0) {
			...
			//Rapes not on the table.
				...
				outputText("<br><br>Your nipples ache with the desire to forcibly breastfeed the gelatinous beast.  Do you?");
		...
		//Not a breastfeeder
			...
			doYesNo(slimeVictoryRape, cleanupAfterCombat);
		}
		cleanupAfterCombat();
	}
	...
	function oozeRapesYouVaginally(): void {
		...
		if (player.cor <= 20) {
			...
			outputText("You arch your back slightly as the slime begins moving, slowly pumping your " + player.vaginaDescript(0) + ".  As it picks up speed a small amount of the thing's greenish fluid builds up around your lower mouth, making your skin tingle and filling you with an unusual arousal.<br><br>");
			//Cocks
			if (player.cockTotal() > 0) {
				//Multi biiiiitches
				if (player.cockTotal() > 1) {
					outputText("Your " + Appearance.multiCockDescript(player) + " rapidly grows erect under this assault and begins to throb.  Having now released yourself to instinct you reach to jerk off, ");
			...
			outputText("After a short time the creature penetrates you fully again and stops, and you realize that at some point you had begun moving your hips with it.  You gasp in surprise as the slime's cock begins churning inside of you, massaging you from the inside in a uniquely pleasurable fashion.<br><br>");
			...
			if (player.cockTotal() > 0) {
				//Multi
				if (player.cockTotal() > 1) outputText("Your " + Appearance.multiCockDescript(player) + " climaxes as well, sending rope after rope of hot, thick cum onto your face and chest.");
				//Single
		...
		//standard result
			...
			//Loose
				...
				if (player.vaginalCapacity() >= 30) outputText(" almost perfectly.<br>");
				else outputText(" perfectly.<br>");
			}
			...
			if (player.cockTotal() > 0) {
				//multi
				if (player.cockTotal() > 1) outputText("Your " + Appearance.multiCockDescript(player) + " climaxes as well, and you open your mouth as you spray rope after rope of hot, thick jism onto your face and chest.  You relish its salty taste, continuing to stroke yourself even as you come.  ");
				//single cock
	...
	function oozeButtRapesYou(): void {
		...
		if (player.cockTotal() > 0) {
			...
			//Multi!
			else outputText("Your " + Appearance.multiCockDescript(player) + " quickly grows erect under the creature's embrace as it pistons into your ass, and before long you stand at full attention.  You weakly rub at a different cock with each hand as it pumps you, and quickly approach an orgasm.  ");
		}
		//Vaginas!
	...
	function serviceLowCorruptionHighLust(): void {
		...
		outputText("  The erection is spongy and pliable to the touch, and with every pulse it seems to fill the cracks between your fingers.<br><br>");
		...
		if (player.vaginas.length > 0) {
			...
			outputText("<br><br>"); //this line present to ensure proper paragraph structure
		}
		//Penises foreplay
		if (player.cockTotal() > 0) {
		...
		if (player.tallness < 82) {
			...
			outputText("A light green fluid begins to coat your hand as you stroke the slime's cock, making your fingers feel warm and sending shivers of arousal through your entire body.<br><br>");
			...
			//female or herm version (if the herm didn't trigger male)
				...
				//Big clitties
				if (player.clitLength >= 2) outputText("You moan and lean back slightly as you grip your " + player.clitDescript() + " between your thumb and forefinger, biting your lip at the intense sensations as you rub your thumb over your " + player.clitDescript() + " and shuddering in anticipation and delight.  ");
				//Small clitties
				...
				if (player.cockTotal() > 0) {
					//Multidicked herms
					if (player.cockTotal() > 1) outputText("You let out a strangled gasp as your body, already straining from exertion, shudders again as your " + Appearance.multiCockDescript(player) + " explodes into orgasm, showering both you and the creature in jizz.  ");
					//Single cocked herms
			...
			outputText("After a few moments the creature slides out from under you, slinking back into the waters of the lake as you recover from the ordeal.  When your wits return, there is nothing to do but stop back by camp.<br>");
		}
		//PHEW! that was a big one.  now...  we do it all again for large players, whee! and this is why this thing is so goddamn long.
			...
			if (player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
				...
				if (player.vaginas.length > 0) {
					...
					outputText("<br><br>It isn't long before you can no longer hold yourself back and with a groan, you explode, showering the creature with your seed as your " + Appearance.multiCockDescript(player) + " and " + player.vaginaDescript(0) + " simultaneously explode into orgasm.  The ooze's penis also seems to swell and a torrent of the green fluid gushes from the tip, creating a large, slippery pool that coats your bottom half and forms a small pool on the ground around you before soaking into the earth.  As you catch your breath the creature's cock begins to lose definition and then recedes as the thing pulls itself back into a blob and slides out from under you, quietly escaping into the nearby water as you watch.  As you get up, you notice a still partly solid blob of green gel on the ground, and pick it up.");
				}
				//Pure male orgasm
			...
			//Female/Herm Masturbation
				...
				if (player.cockTotal() > 0) {
					...
					if (player.biggestTitSize() == 0 || player.breastRows.length == 0) {
						outputText("You begin to moan as the creature rubs the " + player.vaginaDescript(0) + " beneath your " + Appearance.multiCockDescript(player) + ", working you with incredible skill, but sadly ignoring your needy ");
						//multi!
