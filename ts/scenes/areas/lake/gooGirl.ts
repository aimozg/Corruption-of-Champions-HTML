...
class GooGirl extends Monster {
	...
	doAI(): void {
		...
		switch (chooser) {
			case 0:
				...
				break;
			case 1:
				if (this.fatigue <= this.maxFatigue() - 25 && !player.hasStatusEffect(StatusEffects.Bind))
					GooGirl.gooEngulph();
				else
					GooGirl.gooGalAttack();
				break;
	...
	static gooGalAttack(): void {
		...
		//everyone else
			...
			if (monster.hasStatusEffect(StatusEffects.Acid)) {
				...
				if (!player.hasStatusEffect(StatusEffects.Acid)) player.createStatusEffect(StatusEffects.Acid, 0, 0, 0, 0);
			}
			else
	...
	static gooEngulph(): void {
		...
		if (!player.hasStatusEffect(StatusEffects.Bind)) player.createStatusEffect(StatusEffects.Bind, BIND_TYPE_GOO, 0, 0, 0);
		monster.fatigue += 25;
		//combatRoundOver();
	}
	...
	outputDefaultTeaseReaction(lustDelta: number): void {
		if (monster.lust <= 99) {
			if (lustDelta <= 0) outputText("<br><br>The goo-girl looks confused by your actions, as if she's trying to understand what you're doing.");
			else if (lustDelta < 13) outputText("<br><br>The curious goo has begun stroking herself openly, trying to understand the meaning of your actions by imitating you.");
			else outputText("<br>The girl begins to understand your intent. She opens and closes her mouth, as if panting, while she works slimy fingers between her thighs and across her jiggling nipples.");
		}
		else outputText("<br>It appears the goo-girl has gotten lost in her mimicry, squeezing her breasts and jilling her shiny " + monster.skinTone + " clit, her desire to investigate you forgotten.");
	}
...
namespace GooGirlScene {
	//------------
	// SCENES TODO
	//------------
	export function encounterGooGirl(): void {
		clearOutput();
		...
		// Else choose from these options
			...
			if (player.hasCock()) {
				if (player.cocks[player.smallestCockIndex()].cockLength < 24) {
					addButton(0, "DickSex", gooMaleRape, monster, 2);
				}
				if (player.longestCockLength() >= 24) {
					addButton(1, "BigDickSex", gooMaleRape, monster, 1);
				}
				if (player.hasVagina()) {
					addButton(2, "Herm Sex", victoryHermSex, monster);
				}
			}
			else if (player.hasVagina()) {
				addButton(0, "Fem.Fuck", normalFemaleRapesAGooGirl, monster);
				if (gameFlags[PC_FETISH] >= 1) {
					addButton(1, "Exhib.Fuck", exhibitionismGooGirlVictoryRape, monster);
				}
			}
			if (player.hasPerk(PerkLib.Feeder)) addButton(3, "Breastfeed", victoryRapeAGooGalAsFeeder, monster);
			if (player.canOvipositBee()) addButton(4, "Lay Eggs", layBeeEggsInGoo);
			...
			/* UNCOMMENT WHEN WE BEGIN LATEX GIRL SCENES
			 ...
			 addButton(8, "Make Slave", pureGooRecruitmentStart);
			 return;
			 }
			 ...
			 addButton(8, "Make Slave", meanGooGirlRecruitment);
			 }
			 }
			 }
			 */
	...
	//Victory – Male
	function gooMaleRape(monster: GooGirl, type: number = 1): void {
	...
	function victoryRapeAGooGalAsFeeder(monster: GooGirl): void {
		...
		if (player.hasVagina()) outputText("Even without stimulation, your clit practically vibrates under your climax, squeezing your body to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.<br><br>");
		else if (player.hasCock()) if (player.hasVagina()) outputText("Even without stimulation, your cock practically vibrates under your climax, squeezing your body to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.<br><br>");
		else if (player.hasVagina()) outputText("Even without stimulation, your tingling anus practically vibrates under your climax, squeezing to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.<br><br>");
	...
	//[Exhibitionist Perk]
	function exhibitionismGooGirlVictoryRape(monster: GooGirl): void {
		gameFlags[TIMES_FUCKED_NORMAL_GOOS]++;
		clearOutput();
	...
	// CORE DROP
	function coreDropChance(monster: GooGirl): void {
		if (rand(4) == 0 && gameFlags[SLIME_CRAVING] > 0 && !player.hasPerk(PerkLib.SlimeCore) && player.isGoo() && player.gooScore() >= 4) {
			outputText("<br><br>As the goo-girl slithers away, into the lake's placid waves, you notice she seems to have left behind a small blob. Upon investigation, it appears to be a tiny, ruby heart, encased in a slimy " + monster.gooColor8() + " membrane. As you reach to pick it up, the jelly ball quivers and pulses with a warm, cheerful light. Your fingers close on it and the nucleus slides through your palm, into your body!<br><br>");
			
			outputText("There is a momentary pressure in your chest and a few memories that are not your own flicker before your eyes. The dizzying sight passes and the slime core settles within your body, imprinted with your personality and experiences. There is a comforting calmness from your new nucleus and you feel as though, with your new memories, you will be better able to manage your body's fluid requirements.<br><br>");
	...
	export function gooGirlLoss(): void {
		//if (pcCameWorms) {
	...
	function hermLoseToGooGal(monster: GooGirl): void {
		...
		outputText(" in rippling walls of needy slime. Coils of hot and cold run up and down your " + player.cockDescript(x) + ", as playful tongues part your labia and trace every petal-like fold of your " + player.vaginaDescript(0) + ", filling you to your brim with gooey, thirsty ooze.<br><br>");
		...
		outputText("  Your spurting ecstasy floods the girl's jelly body, coloring her " + monster.gooColor3() + " body white as she drinks deeply, coaxing a few seconds of gasping spasms into long, seed-spewing minutes of climaxing euphoria.");
	...
	function femaleLoseToGooGal(monster: GooGirl): void {
		...
		//[If the player isn't pregnant]
			...
			outputText("  Your body is wildly distorted by the intruder, some aspect of her slimy coating anesthetizing your anticipated pain with tooth-grinding pleasure. You stroke your bloated form, thoughts turning to sweet daydreams of pregnancy, your body full of a pulsing, liquid life, dependent upon you and squirming with eager desire.<br><br>");
			...
			if (player.pregnancyIncubation >= 1) outputText("  Everything she pumped into you runs back out after.  For some reason or another, it didn't take.");
				...
				player.knockUp(PregnancyType.GOO_GIRL, INCUBATION_GOO_GIRL, 1, 1);
			}
		}
		player.dynStats("sens", 4);
		player.slimeFeed();
