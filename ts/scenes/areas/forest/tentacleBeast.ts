...
class TentacleBeast extends Monster {
	...
	static tentaclePhysicalAttack():void {
		outputText("The shambling horror throws its tentacles at you with a murderous force.<br>");
		let temp = ((monster.str + monster.weapon.attack) - Math.random() * (player.tou) - player.armor.defense);
		if (temp < 0) temp = 0;
		//Miss
	...
	static tentacleEntwine():void {
		outputText("The beast lunges its tentacles at you from all directions in an attempt to immobilize you.<br>");
//Not Trapped yet
		if (gameFlags[TENTACLE_BIND] == 0) {
			...
			//Fail
				...
				outputText("While you attempt to avoid the onslaught of pseudopods, one catches you around your " + player.foot() + " and drags you to the ground. You attempt to reach for it to pull it off only to have all of the other tentacles grab you in various places and immobilize you in the air. You are trapped and helpless!!!<br><br>");
				//Male/Herm Version:
				if (player.hasCock()) outputText("The creature, having immobilized you, coils a long tendril about your penis. You shudder as the creature begins stroking your cock like a maid at a dairy farm in an attempt to provoke a response from you. Unable to resist, your " + player.cockDescript(0) + " easily becomes erect, signaling to the creature that you are responsive to harsher stimulation.<br>");
				//Female Version:
				else if (player.hasVagina()) outputText("The creature quickly positions a long tentacle with a single sucker over your clitoris. You feel the power of the suction on you, and your body quickly heats up.  Your clit engorges, prompting the beast to latch the sucker onto your " + player.clitDescript() + ".<br>");
				//Genderless
				else outputText("The creature quickly positions a long tentacle against your " + player.assholeDescript() + ". It circles your pucker with slow, delicate strokes that bring unexpected warmth to your body.<br>");
...
namespace TentacleBeastScene {
	...
	function tentacleVictoryRape(): void {
		...
		if (player.gender == 1 || player.gender == 3) {
			outputText("Seizing the opportunity, you rush the monster while it is stopped. You grab the fattest hollow tentacle you can find and summarily shit-kick the beast onto the ground. Holding the creature down with one foot, you take the hollow tentacle and poise it in front of your raging erection.<br><br>");
			outputText("\"<i>You want dick milk, you freak?!</i>\" you bellow in triumph. \"<i>HERE YOU GO!</i>\"<br><br>");
			outputText("You impale the tentacle on your penis and begin humping wildly, treating the creature's armature as your own personal onahole. The creature squirms wildly, trying to escape your lust-driven strength. Replying with a swift kick with your free foot, the creature's resolve to escape wavers.<br><br>");
			outputText("\"<i>Quit fuckin' squirming and take your MEDICINE!</i>\" you thunder as you cheap-shot the beast for good measure.<br><br>");
			outputText("Feeling your peak approach, you allow your muscles into their rhythmic contractions as you unload your cum into the creature like a howitzer attacking a fort. Laughing like a maniac with each shot, you see your jizz leak out as the creature struggles to assimilate your 'donation'.<br><br>");
		...
		//Female:
			...
			//High Corruption
			if (player.cor >= 66) {
				outputText("Seizing the opportunity, you rush the monster while it is stopped. You grab the fattest phallic tentacle you can find and summarily shit-kick the beast onto the ground. Holding the creature down with one foot, you take the tentacle-cock and poise it in front of your dripping cunt.<br><br>");
				outputText("Laughing like a true psychotic, you stuff the tentacle into your womb.");
				player.cuntChange(20, true, true, false);
				outputText("  Your vaginal muscles quickly go to work stroking, squeezing and kneading the appendage. The creature, more intent with escape than hammering your box, begins struggling. You summarily slug the beast as well as any professional pugilist to stop its throes.<br><br>");
				outputText("\"<i>STOP STRUGGLING AND FUCK MY LITTLE PUSSY!</i>\", you screech.<br><br>");
			...
			//Rape Win Female-Low Corruption:: 
				...
				outputText("Seizing the opportunity, you rush the monster while it is stopped. You grab the fattest phallic tentacle you can find and summarily push the beast onto the ground. Holding the creature down with your body weight, you take the tentacle-cock and poise it in front of your dripping cunt.<br><br>");
	...
	export function tentacleBeastLoss(): void {
		if (player.HP <= 0) {
	...
	function tentacleLossRape(): void {
		...
		if (player.countCocksOfType(CockTypesEnum.HORSE) > 0 && player.cocks[0].cockLength > 15 && player.cocks[0].cockThickness >= 3) {
			if (player.cor < 75 && player.lust < 100) outputText("It grabs you before you can get away!<br><br>While you attempt to resist the abomination, its raw muscle mass is too much. ");
			...
			if (player.balls > 0) outputText("Your balls have overfilled the creature!<br><br>");
		...
		if (player.hasCock() && player.cumQ() < 1500 && player.ass.analLooseness < 4) {
			...
			player.dynStats(["tou", 1], ["inte", -.5], ["lib", 2], ["sens", 1], ["cor", .5]);
			if (inCombat()) {
				cleanupAfterCombat();
				player.orgasm();
			}
		...
		if (player.biggestLactation() >= 3.5 && player.gender > 0) {
			...
			if (player.gender == 2) {
				...
				if (player.vaginalCapacity() < 30) outputText("The beast senses your excitement and with beguiling speed swiftly impales your " + player.vaginaDescript(0) + " with one of its massive, knobbly tentacles.  You squeal in pain and pleasure as you feel every bumpy inch pound into you, your cunt being stretched to unbelievable proportions.  The tentacle quickly bottoms out in your shallow hole, pressing urgently against your cervix as it begins to rhythmically pound your " + player.vaginaDescript(0) + ".<br>");
		...
		if (player.gender == 1) {
			...
			outputText("<br><br>Realizing what is about to happen, you try to struggle. The beast responds by slamming you to the ground a few times, stunning you.  ");
		...
		if (player.gender == 3) {
			...
			outputText("<br><br>Realizing what is about to happen, you try to struggle. The beast responds by slamming you to the ground a few times, stunning you. In your daze you see a monstrous, hollow tentacle poised over your ");
	...
	function tentacleRapeContinuation(): void {
		...
		// Female (Kicks to special scene)
			...
			outputText("<br><br>In a coordination that can only signify higher intelligence, the monster fucks your " + player.vaginaDescript(0));
			if (player.vaginas.length > 1)
				outputText("s");
	...
	function centaurLossGendered(): void {
		...
		if (player.hasVagina()) {
			...
			outputText("  You feel the beast bottom out against your uterus and cry out in pleasure, gyrating yourself about as fluid sprays behind you.<br><br>");
		}
		...
		if (player.ass.analLooseness >= 4) {
			...
			outputText("  You moan into the tentacle in your mouth appreciatevely at the beast's spectacular skill.<br><br>");
		}
	...
	export function encounter(): void {
		...
		if (player.gender == 0 && rand(3) == 0 && !player.isNaga() && !player.isTaur() && !player.isGoo()) {
			...
			outputText("You see a massive, shambling form emerge from the underbrush. While first appearing to be a large shrub, it shifts its bulbous mass and reveals a collection of thorny tendrils and cephalopodic limbs. Sensing your presence, it lumbers at you, full speed, tentacles outstretched.<br><br>");
			...
			outputText("Completely confused, all you can do is sit there in shock.<br><br>\"<i>Where are your naughty bits, goddammit!</i>\" the creature bellows. \"<i>Us tentacle creatures need to FEED!</i>\"<br><br>");
			outputText("You sheepishly state that you are gender-neutral and have no genitalia.<br><br>\"<i>You gotta be shitting me!!</i>\" the monster bellows in contempt. \"<i>Of all the motherfuckers I ambush, it has to be the ONE bastard I can't feed from! What am I supposed to do now, asshole?! I gotta eat!</i>\"");
		...
		//Combat starter
		outputText("You see a massive, shambling form emerge from the underbrush.  While it resembles a large shrub, a collection of thorny tendrils and cephalopodic limbs sprout from its bulbous mass.  Sensing your presence, it lumbers at you, full speed, tentacles outstretched.<br><br>");
		if (player.cor > 50 && player.cor <= 75)
			outputText("You debate the merits of running from such a creature.<br><br>");
		if (player.cor > 75)
			outputText("You smile and stride forward, welcoming the pleasure you expect from such a monster.<br><br>");
		//Worms get nothing!
	...
	function tentacleLossGenderless(): void {
		...
		if (player.isTaur()) {
			...
			outputText("\"<i>Where are your naughty bits, goddammit!</i>\" the creature bellows. \"<i>Us tentacle creatures need to FEED!</i>\"<br><br>");
			outputText("You sheepishly state that you are gender neutral and have no genitalia.<br><br>\"<i>You gotta be shitting me!!</i>\" the monster bellows in contempt. \"<i>Of all the motherfuckers I ambush, it has to be the ONE bastard I can't feed from! What am I supposed to do now, asshole?! I gotta eat!</i>\"");
			...
			outputText("<br><br>The beast slaps you squarely on the ass as if to push you along. \"<i>Get the fuck out of here!</i>\" it screams.  \"<i>Get lost so I can hunt me a REAL meal!!!</i>\"  ");
			outputText("You walk away from the creature, which hides back in the brush. After you trek a bit, you wonder if what happened really DID happen...");
	...
	function centaurLossGenderless(): void {
		clearOutput();
		//TODO spriteSelect(100);
		...
		if (gameFlags[TENTACLE_GENDERLESS_CENTAUR] == 0 || player.balls == 0) {
			...
			outputText(", but stops when it reaches your haunches.  Another starts testing the same area, briefly touching your " + player.assholeDescript() + " but clearly not finding what it's looking for.<br><br>");

			outputText("\"<i>WHAT THE FUCK IS WRONG WITH YOUR BODY?!</i>\" yells out an unnervingly human voice.<br><br>");

			outputText("Startled past horror, your mouth hangs wide open.<br><br>");

			outputText("\"<i>Why the FUCK can't I find your juicy bits?</i>\" the creature shrills.  \"<i>I'm so hungry I could risk stealing spoo from an army of goblins in heat!</i>\"<br><br>");

			outputText("You stammer out something about having no genitals, not thinking clearly enough to dissemble.<br><br>");

			outputText("\"<i>Oh, you think this shit is FUNNY, don't you?</i>\"  The voice has switched to a mocking tone.  \"<i>I know, let's wander into the forest and fuck with the hungry creatures who want some nice, nutritious cum!  Let's make them work for my amusement!  It'll be fucking HILARIOUS!</i>\"<br><br>");

			outputText("A tentacle smacks your " + player.buttDescript() + " hard, and the voice returns to normal.<br><br>");
			...
			outputText("\"<i>FUCK you, you stupid horse!  Why don't you grow a pair?  Literally!</i>\"<br><br>");
		...
		//(Followup scene, if pc has seen above at least once, is unsexed centaur and has balls: -Z)
			...
			outputText(", slipping forward to probe between your haunches.  It arrives at and discovers your " + player.sackDescript() + " with some little ceremony, stroking and fondling it.<br><br>");

			outputText("\"<i>Now THIS is what I'm talking about!</i>\" the creature's eerie voice sings out.  \"<i>Daddy needs his medicine!</i>\"<br><br>");

			outputText("The tentacle, now joined by a second, hunts around your " + player.ballsDescriptLight() + ", seeking any organs that might serve as a release valve for their contents.  You stare at it as it searches, quite certain you know what's coming next.<br><br>");

			outputText("\"<i>No, no, no. Where the FUCK is it?</i>\" the creature mumbles, frustration spiking the pitch of its voice.<br><br>");
	...
	function nagaLossGenderless(): void {
		clearOutput();
		// TODO spriteSelect(100);
		...
		outputText("An oddly human voice comes from the undergrowth, catching you off-guard.  \"<i>Look, I'm really sorry about this, but I'm really not all that familiar with, uh, whatever it is you are.  Where do you keep the naughty bits?</i>\"<br><br>");

		outputText("A little stunned by the question, you tell the voice that you don't have any \"<i>naughty bits</i>\".<br><br>");

		outputText("\"<i>I'm sorry, maybe I just worded the question badly.  Um, where do you keep your penis... esss and or vagina... ssss.</i>\"  The words are followed up by prolonged hisses that may or may not represent the usual attempt to transmute one language to another by tacking new suffixes on.<br><br>");

		outputText("Sensing an opportunity to get out of this situation, you respond with your own series of hisses and hand gestures as if to say you have no idea what the beast wants.  It responds with a sigh and you're released from its grip, landing on the ground in a bit of a heap.<br><br>");
	...
	function gooLossGenderless(): void {
		clearOutput();
		// TODO spriteSelect(100);
		outputText("All of a sudden, tentacles come whipping out of the undergrowth to grab you.  Though, they're moving a little too fast, and manage to compress your body walls so far together that you're almost squeezed in half.<br><br>");

		outputText("\"<i>SHIT. SHIT. SHIT. SHIT.</i>\"  An oddly human voice is profaning loudly.  \"<i>Are you dead?</i>\"<br><br>");

		outputText("You respond that you are not, you're just mostly liquid and insubstantial.<br><br>");

		outputText("\"<i>Uh huh... that so?  Well, so long as you have some substantial naughty bits, I'll be happy.</i>\"<br><br>");

		outputText("There's an awkward silence.<br><br>");

		outputText("\"<i>You haven't got anything, have you?</i>\"<br><br>");

		outputText("You shake your head.<br><br>");

		outputText("\"<i>Well, fuck.</i>\"  A tentacle pokes you and you'd guess the beast is watching you jiggle as it chuckles.  \"<i>Maybe this isn't a total waste. I wonder, what do you taste like?</i>\"<br><br>");

		outputText("One of the larger tentacles extends and latches onto your base, its anus-like opening sucking gently at your gooey mass.  There follows a brief moment where you're not really afraid of the situation, but are instead mildly curious yourself what you taste like.<br><br>");

		outputText("\"<i>FUCK!</i>\" comes the voice again.  \"<i>You're sour apple!  I fucking HATE sour apple!</i>\"<br><br>");
	...
	function futaTentacleBadEnd(): void {
		...
		outputText("In your last moments of lucidity, you realize that you are not being eaten or technically harmed at all. The creature has bonded with you as a living producer of food.  As long as you are healthy and cumming, it has all the food it could ever possibly want... so long as your gonads hold out.<br><br>");
	...
	function futaTentacleEpilogue(): void {
		...
		if (gameFlags[GIACOMO_MET] > 0) {
			outputText("\"<i>Well, well, well. You aren't a total loss, I see.</i>\", says a sharp, masculine voice.<br><br>");
			outputText("While the fog of your brain has yet to lift completely, you recognize the voice to be the seedy merchant, Giacomo.<br><br>");
			outputText("\"<i>It is a good thing I happened to be out and about today.</i>\", Giacomo says. \"<i>I was testing out a new weapon to sell and I happened to see one of those nasty tentacle beasties. I had no idea they captured prey! Hell, you must have spent a few months inside that thing feeding it!</i>\"<br><br>");
			outputText("You attempt to say something, only to find yourself incapable of speaking. You feel the man's bony hands pick you up and set you down in what feels like his cart.<br><br>");
			outputText("\"<i>Well, I can't be a total bastard all the time.</i>\", Giacomo jingles. \"<i>I guess I can drop you off at the next village I come to so you can recover. Isn't that absolutely nice of me! Even better! I will do this for free!!!</i>\"<br><br>");
		...
		//[Never met Giacomo]
			...
			outputText("\"<i>Will " + player.mf("he", "she") + " live?</i>\", says a soft and feminine voice.<br><br>");
			outputText("\"<i>Yes, doctor. " + player.mf("He", "She") + " will live.</i>\", replies a gruff and clearly masculine voice.<br><br>");
			outputText("\"<i>Is the beast dead</i>\", queries the doctor.<br><br>");
			outputText("\"<i>Dead several times over, madam.</i>\", answers the man.<br><br>");
			outputText("\"<i>We cannot leave this unfortunate " + player.mf("man", "woman") + " out in the wild like this. Load " + player.mf("him", "her") + " onto the wagon. We will take " + player.mf("him", "her") + " back to the village. I am certain I can help this " + player.mf("man", "woman") + " recover.</i>\", the doctor states flatly.<br><br>");
	...
	/*
	 ...
	 if (player.gender == 1 || player.gender == 3) {
	 if (rand(2) == 0) {
	 outputText("In an effort to distract the creature, you begin gyrating your hips and swinging your penis in a shameless imitation of MeatSpin. The Tentacled Horror briefly pauses to observe your actions and rears similar to a posturing spider, considering your next actions.<br><br>");
	 monster.lust += 10 + rand(5);
	 }
	 //Failure:
	 else
	 outputText("You grab your penis and shake it feverishly at the creature in order to distract it.  It swats a tentacle at you, forcing you to adroitly dodge the reprisal.  Apparently, the beast found you unimpressive.<br><br>");
	 }
	 ...
	 if (rand(2) == 0) {
	 //GENDERLEZZ
	 if (player.gender == 0) outputText("You brazenly turn your back on the creature and, glancing over your shoulder, begin bending over and presenting your " + player.buttDescript() + " to the beast. It pauses and observes while you bend over further, presenting a full view of both your back door and your " + player.assholeDescript() + ". You shift from side to side and observe the beast match your movements. You have obtained its attention to say the least.<br><br>");
	 //CHICKS
	 else outputText("You brazenly turn your back on the creature and, glancing over your shoulder, begin bending over and presenting your " + player.buttDescript() + " to the beast. It pauses and observes while you bend over further, presenting a full view of both your back door and your honey hole. You shift from side to side and observe the beast match your movements. You have obtained its attention to say the least.<br><br>");
	 monster.lust += 10 + rand(5);
	 }
	 //Failure
	 else {
	 outputText("You begin shaking your hips and grabbing your " + player.allBreastsDescript() + " to distract the creature. However, the near-miss from the tentacle it attempted to swat you with convinces you of its desire to beat your ass, rather than fuck it.<br><br>");
	 }
	 }
