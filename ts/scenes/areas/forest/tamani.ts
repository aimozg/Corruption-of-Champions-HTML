...
class Tamani extends Monster {
	constructor() {
		...
		this.sens       = 65;
		this.cor        = 50;
		this.weapon     = Items.monsterWeapon("fists", 0, "tiny punch");
		this.armor      = Items.monsterArmor("leather straps");
		this.bonusHP    = 40;
		this.lust       = 40;
		this.lustVuln   = 0.9;
...
//Changes Tamani chest description depending on how much she's been impregnated.
	export function tamaniChest(): string {
		...
		switch (gameFlags[TAMANI_TIMES_IMPREGNATED]) {
			case -1:
			case  0:
				descript = (rand(2) == 0 ? "ample " : "handful of ");
				break;
			case  1:
			case  2:
			case  3:
				descript = (rand(2) == 0 ? "large " : "jiggling ");
				break;
			case  4:
			case  5:
			case  6:
				descript = (rand(2) == 0 ? "wobbling " : "impressive ");
				break;
			case  7:
			case  8:
			case  9:
			case 10:
				descript = (rand(2) == 0 ? "over-proportioned " : "super-sized ");
				break;
			default:
				descript = (rand(2) == 0 ? "massive " : "gigantic ");
				break;
		}
		switch (rand(11)) {
			case  0:
	...
	export let tamanipreg = new Tamani(); //Constant instance of Tamani solely for pregnancy tracking. There may be a better way to do this.
	//------
	// VICTORY AND DEFEAT SCENES
	//------
	
	//WIN
	export function tamaniWin(): void {
		clearOutput();
		if (monster.HP <= 0) {
	...
	// Victory Menu
	function tamaniVictoryMenu(): void {
		...
		if (player.lust >= 33 && player.totalCocks() > 0 && gameFlags[SFW_MODE] <= 0) {
			...
			if (player.cockThatFits(monster.analCapacity()) >= 0) addButton(1, "Buttfuck", tamaniAnalSex);
			...
			if (!tamanipreg.isPregnant() && player.canOvipositSpider()) addButton(2, "Lay Eggs", tamaniBeaten);
			if (gameFlags[TAMANI_DEFEAT_COUNTER] >= 4 && monster.HP <= 0) addButton(3, "NO MORE!", killTamaniChoice);
	...
	function tamaniSexWon(): void {
		...
		if (player.cockArea(x) <= 90) {
			...
			outputText("It's too tempting of a target to resist.  You open your " + player.armor.equipmentName + " and allow your " + player.multiCockDescriptLight() + " to flop free.  You're already hard from the enticing display, and in a moment you're pressing against her lust-slicked pussy");
			...
			if (player.totalCocks() > 1) outputText("s");
			outputText(" clenching and squeezing around you quickly washes it away.<br><br>");
			...
			if (player.totalCocks() == 1) outputText("pussy squeezing tightly as she cums hard.<br><br>");
			else outputText("holes squeezing tightly as she cums hard.<br><br>");
			...
			outputText("<br><br>Tamani winks, sucking on her finger as she staggers up.  She coos, \"<i>Mmmm, Tamani loves it when her " + player.mf("sexy lover", "stud") + " dominates her.  Do you think your ");
			if (player.balls > 0) outputText("sexy balls have ");
			else outputText("sweet cock has ");
			outputText("any more cream to give to me?</i>\"<br><br>");
			outputText("She doesn't give you a chance to answer as she wobbles off, jiggling pleasantly in all the right places, \"<i>Of course you do.  I'll be back for the rest later!</i>\"<br><br>");
			if (inCombat()) {
				cleanupAfterCombat();
				player.orgasm();
			} //TODO TEST THIS
		...
		//Too big? Jerk off with feet and bukkake
			...
			outputText(", so you grab her by the ankles and wrap her soft-soled feet about yourself.   You start jerking yourself off, using Tamani as a cute but expendable masturbation aid.  She chews a fingernail and massages her " + tamaniChest() + " as she watches you, doing her best to put on a show.  The little slut seems to like it.<br><br>");
			outputText("Her feet start to grow slick with your sweat and pre-cum, sliding effortlessly along the length of your shaft as you continue to bring yourself towards orgasm.  You watch while Tamani dips her fingers into the slick folds of her hungry cunt, getting off on being used in such a perverse manner.  She coos, \"<i>How do my feet feel " + player.mf("stud", "slut") + "?  Are they soft and slick when they slide on your cock?  Are you going to cum for Tamani and paint her white?  You are.  You know you are.  Cum for Tamani.</i>\"<br><br>");
			...
			outputText("has any more cream to give to me?</i>\"<br><br>");
			
			outputText("She doesn't give you a chance to answer as she staggers up and strikes a sexy pose, \"<i>Of course you do.  I'll be back for the rest later!</i>\"<br><br>");
	...
	function killTamaniChoice(): void {
		clearOutput();
	...
	function postTamaniRemoval(): void {
		...
		player.createKeyItem(KeyItems.TamanisSatchel, 2, 1, 1, 100); // GAIN SATCHEL
	...
	//LOSE
	export function tamaniLoss(): void {
		if (player.HP <= 0) {
			if (player.totalCocks() > 0) {
				...
				clearOutput();
				outputText("Tamani sighs as you begin to lose consciousness, \"<i>You dummy, why'd you get rid of the fun parts?</i>\"");
				cleanupAfterCombat();
			}
		} else {
			if (player.totalCocks() > 0) {
	...
	function tamaniSexLost(): void {
		...
		if (player.cocks[0].cockThickness >= 5) outputText("Impossibly, despite your titanic girth, the ring somehow expands to squeeze onto you, though it's painfully tight.<br><br>");
		else outputText("Somehow the ring fits onto you perfectly, as if it was made exactly for your " + player.cockDescript(0) + ", though it remains painfully tight.<br><br>");
		...
		if (player.biggestTitSize() >= 6) outputText("but can't manage to see your bloated prick past the wall of titflesh.<br><br>");
		else outputText("at your bloated prick, looking almost overinflated from all the blood that tight ring has forced into it.<br><br>");
		outputText("<b>*SMACK*</b>  Tamani's hand cracks against your " + player.buttDescript() + " making you lurch and bite back a cry of pain.   \"<i>Bad boys get punished!</i>\" she yells as she brings her hand down again, slapping your other cheek.  You're sure there must be two tiny red hand-prints forming already. The kinetic impacts of her blows vibrate through your hips, traveling into your sensitive " + player.cockDescript(0) + " and working with the magic-ring to spread equal measures of pain and pleasure through its length.<br><br>");
		...
		outputText(" and asks, \"<i>So are you sorry for making me wait for my cream " + player.mf("stud", "slut") + "?  If you are, just say <b>I'm sorry mistress Tamani, please milk my cock whenever you want</b> and then beg me to cum.</i>\"<br><br>");
		...
		if (player.cor < 50) outputText("  You have no choice, you'll say what she wants you to say.<br><br>");
		else outputText("  Being denied like this was kind of fun, but you REALLY need to get off now, so you'll say what she wants you to say.<br><br>");
		...
		if (player.cor > 50) outputText("blushing hotly and doing your best to look like a submissive toy.<br><br>");
		else outputText("blushing in shame and embarrassment, but meaning every word.<br><br>");
	...
	function tamaniSexLetHer(): void {
		...
		if (inCombat()) {
			...
			if (player.isTaur()) {
				...
				outputText("Tamani sidles up to you and pats your flank, cooing, \"<i>There there horsey, everything's going to be okay.  Momma Tamani's going to take really good care of you.</i>\"  Before you know it your " + player.armor.equipmentName + " is on the ground and the goblin is underneath you, climbing up to your crotch.<br><br>");
			}
			//Not taur
			else {
				...
				outputText(".  She strips you down, giving you a smirk as she says, \"<i>Just lie there and I'll take what I need, babe.</i>\"<br><br>");
			}
		}
		//Voluntary submission
			...
			//Taur
			if (player.isTaur()) {
				outputText("You meekly undress and ask Tamani how she would like to take you today.  She titters and slowly walks around you, taking her time, taking in your formidable bestial body and your " + player.multiCockDescriptLight() + ". \"<i>All this, for little ol' me,</i>\" she breathes. She sounds more tantalized than daunted. \"<i>Stand still for me, horsie,</i>\" she commands. Unable to see her any more behind your flank, you do as she says. You feel a pair of tiny hands on your back thigh and then a small but dense weight climbing steadily towards your groin. You bite back laughter at the ticklish sensation.<br><br>");
			}
			else {
		...
		if (player.isTaur()) {
			...
			if (player.cocks[0].cockLength <= 18) {
				...
				//No balls:
				if (player.balls == 0) outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics.<br><br>");
				else outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics, before the goblin in question drives all thoughts out of your head by mashing her soft soles into your " + player.ballsDescriptLight() + ".<br><br>");
				...
				if (player.balls == 0) outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics.<br><br>");
				else outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics, before the goblin in question drives all thoughts out of your head by beginning to mash her soft soles into your " + player.ballsDescriptLight() + ".<br><br>");
				...
				outputText("\"<i>Go on then,</i>\" she breathes eventually. \"<i>I guess you've had enough. Cum for Mistress Tamani, " + player.mf("stud", "slut") + ". Cover me from head to toe.</i>\" As she says this, she walks her hands down your belly, positions herself directly over your head, uses her own lubrication to rub her feet furiously down your " + player.cockDescript(0) + " and then, with one last evil giggle, sticks her tongue directly into your urethra.<br><br>");
				
				//Low cum:
				if (player.cumQ() < 1200) {
					outputText("With a ragged sigh, you reach your peak. Your first blast of jizz goes straight into Tamani's mouth, but you feel the goblin nimbly and hastily changing her position so that the very tip of your " + player.cockDescript(0) + " can feel her moist snatch. Being teased for so long makes the sensation incredible, and it feels like entire minutes go by whilst you stand there, sweat dripping off you as you cum over and over again until your cock is doing nothing but flexing mindlessly.<br><br>");
				...
				//High cum:
					...
					outputText("With a bellowing roar, you cum with mind blowing force. Your churning balls blast out a river of gooey fluid, the first glorious arc of which flies straight past your human front and paints the tree in front of you. Totally insensate, you can do nothing for entire minutes but stand there and cum, over and over again, sweat dripping off your frame, until the entire area underneath you is covered with your spooge.<br><br>");
		...
		//Not-taurs
			...
			if (player.cockArea(0) <= 90) {
				...
				outputText("Moaning like a whore, she easily slips the rest of the way down, bottoming out her sopping-wet fuck-tunnel.<br><br>");
				...
				else outputText("Your " + player.nippleDescript(0) + "s burn with pain as the goblin twists and pulls on them, abusing them.  Pleasure and pain mix together, overwhelming you, reducing your world to little more than the slap of genital against genital and the painful torture happening to your chest.<br><br>");
				outputText("Tamani plays your body like a fiddle, keeping you near the edge as she noisily orgasms over your shaft again and again.  You can tell she's starting to tire, so she gives you a kind look and begins to pick up the tempo.   She whispers softly, \"<i>Go ahead, cream my honeypot until I can barely walk.</i>\"<br><br>");
			...
			//(Doesnt fit)
				...
				outputText(" a rough slap.  Whimpering, you still your rebellious hips and allow her to play you like the world's most sensitive fiddle.  Tamani pulls your " + player.cockHead() + " up and rocks herself against it, openly moaning as you dribble pre into her sopping gash.<br><br>");
				...
				outputText(", and places the dildo's leaking tip against your " + Appearance.assholeOrPussy(player) + ".  You feel it stretching you out, reacting with your body's own fluids to swell up inside you like a pervert's balloon as Tamani pushes it further and further inside you.");
				if (player.hasVagina()) {
					outputText("  ");
				...
				outputText("  A burning heat radiates through your body, making your " + player.nippleDescript(0) + " tingle and grow ever more sensitive.<br><br>");
				outputText("Tamani rocks back and forth, sliding herself against your " + player.cockDescript(0) + ", humping it lewdly as your veins rub her hard little nub of a clit.  She pants, making sure to keep giving you sultry looks over her shoulder, licking her glossy lips as she brutally teases you.  You can feel her getting off over and over as she grinds, sliding faster and faster on your shaft.   For your part, your " + player.cockDescript(0) + " begins making a puddle of pre-cum on your belly as the dildo's drug-like cum affects you, making you feel horny and hot.<br><br>");
				outputText("The kinky goblin dominatrix on your " + player.cockDescript(0) + " leans even further forwards, angling the dildo in your " + Appearance.assholeOrPussy(player) + " upwards to squeeze your ");
				...
				outputText(", \"<i>let out all that hot sticky spunk for Mistress Tamani.</i>\"<br><br>");
				outputText("Your body immediately and happily complies, squirting a jet of thick white jism onto the goblin's jiggling ass-cheeks.  She slides down, aligning her cunt with your swollen cock-tip, taking the next squirt directly between her lips as she rubs against you.  Tamani slaps the dildo in your " + Appearance.assholeOrPussy(player) + " gently, squeezing out an incredibly thick gout of semen as she stretches you wider.  ");
				if (player.cumQ() >= 250) {
				...
				outputText("The dildo slips out of your abused " + Appearance.assholeOrPussy(player) + " forgotten in the heat of your lovemaking, leaking pink goo.<br><br>");
				outputText("Tamani eases up off of you, dripping a mixture of sexual fluids and stretching as if she had just completed a long workout.  The slutty goblin winks at you and waves, \"<i>Thanks for the cum.  Be sure and take some time to refill.  I wanna be soaked again next time!</i>\"<br><br>");
	...
	function tamaniHypnosis(): void {
		//TODO TAMANI SPRITE spriteSelect(56);
		clearOutput();
		//Find a dick that fits
		const primary = player.cockThatFits(65);
		let secondary = 0;
		const cocks   = player.totalCocks();
		
		outputText("Tamani crooks her finger and you come running, already feeling yourself straining against your " + player.armor.equipmentName + " for a chance to fuck your wife and mistress.  She giggles, \"<i>Ohhh, how sweet, you're so ready to do your duty.</i>\"<br><br>");
		
		outputText("The goblin traces her fingers over the bulge in your " + player.armor.equipmentName + " before reaching inside of it to give you a tender squeeze.  You sigh, happy to have earned such tender treatment from your lovely wife.  Automatically, your hands undo your " + player.armor.equipmentName + ", removing the troublesome garment so as not to interfere with your lovemaking.   Tamani gives you another squeeze and teases, \"<i>Oh baby, you're hooked on Tamani's pussy, aren't you?  I can tell, you're practically trembling.  Let Tamani help take care of that for you.</i>\"<br><br>");
		
		outputText("She smirks and bends over, looking back up at you from between her legs.  She offers, \"<i>Go ahead and stick it in.  A horny " + player.mf("boy", "herm") + " like you needs to have that delicious cock inside a nice, tight wife like myself.</i>\"<br><br>");
		//[FITS]
		if (primary >= 0) {
			...
			outputText("  You don't stop until your " + player.cockHead(primary) + " is the only thing inside her.<br><br>");
			...
			outputText("  She visibly orgasms, shaking and trembling from head to toe as her pussy milks your " + player.cockDescript(primary) + ".  It only lasts a few seconds, though they feel like heaven, before her body goes nerveless.<br><br>");
			...
			if (player.skinType == SkinType.FUR) outputText("matted");
			...
			outputText("  Swelling wider inside the wet hole, your " + player.cockDescript(primary) + " tingles and clenches, about to unload.<br><br>");
		...
		//[DOESNT FIT]
			...
			outputText(player.mf("strong man's", "sexy lover's") + " " + Appearance.cockNoun(CockTypesEnum.HUMAN) + "?</i>\"<br><br>");
			...
			outputText(" her not to stop.  Tamani starts to giggle, but bursts into a moan as her fingers slide along her easily visible clit. She pants, \"<i>Hurry up and come for Tamani baby, I wanna feel you pumping your cum inside me!</i>\"<br><br>");
	...
	export function encounterTamani(): void {
		...
		//Dudezillaz:
			...
			switch (tamanipreg.pregnancyEventNum) {
				...
				default:
					tamaniMaleRepeatEncounter();		//She's not pregnant or is only slightly pregnant
			}
	...
	function tamaniFemaleEncounter(): void {
		...
		outputText("She says, \"<i>There's only so much cock around, and I got dibs on ALL of it, O.K. skank?</i>\"<br><br>");
		//[Umm OK?] [No]
		menu();
		addButton(0, "Yes", tamaniFemaleYes);
	...
	function tamaniFemaleYes(): void {
		//spriteSelect(56); TODO Tamani Sprite
		clearOutput();
		outputText("\"<i>That's what I thought,</i>\" says the goblin as she fishes around in her pouches, \"<i>but I'm not cruel, I'll give you my best dildo so you can keep your hot little box stuffed all the time.</i>\"<br><br>");
		...
		if (player.cor < 50) outputText("Gross.<br><br>");
		else outputText("Getting cock-slapped would've been kind of hot...<br><br>");
		outputText("The goblin leaves you with a warning, \"<i>Be careful, it likes to leak aphrodisiacs like crazy.  Believe me, those are FUN to get addicted to.  Oh, and remember – Tamani owns all the cocks around here, so if you ever grow one, come pay your dues!</i>\"<br><br>");
		outputText("(<b>Deluxe Dildo acquired!</b>)");
		player.createKeyItem(KeyItems.ToyDeluxeDildo, 0, 0, 0, 0);
		doNext(Camp.returnToCampUseOneHour);
	}
	...
	function tamaniFemaleNo(): void {
		//spriteSelect(56); TODO Tamani Sprite
		clearOutput();
		outputText("The goblin harrumphs, \"<i>We'll see about that slut.  I'll be knocked up from every monster and stud this side of the mountain.  Hell, just grow one dick, and see how fast Tamani's legs get wrapped around you!</i>\"<br><br>");
	...
	function preferTamaniFemdom(): void {
		...
		addButton(1, "Refuse", declineTamaniFacesits);
	}
	
	// Decline Femdom
	function declineTamaniFacesits(): void {
		clearOutput();
	...
	function tamaniFirstTimeConsentual(): void {
		...
		outputText("Her hips sway seductively as she approaches with her right hand dipping into the moist honeypot of her sex.  You disrobe, throwing your " + player.armor.equipmentName + " to the side before you reach down and lift her up, pressing her curvy body against your ");
		if (player.biggestTitSize() > 1) outputText(player.allBreastsDescript());
		else outputText("chest");
		outputText(".  She wraps her tiny arms around your neck and kisses you passionately, letting her tongue slither through your lips.   The two of you french kiss hard, virtually tongue-fucking each other.<br><br>");
		outputText("She breaks the kiss and smiles, licking the shining purple lipstick she wears as she whispers in your ear, \"<i>Mmmhmm, I knew your juicy cock just couldn't resist a wet and ready pussy like mine.  I made sure to lace my lipstick with fertility enhancing chemicals too, so we'll be nice and messy.</i>\"<br><br>");
		...
		outputText("  You NEED to fuck her pussy full – NOW.<br><br>");
		//(FITS)
		if (player.cockArea(0) <= 90) {
		...
		//(TOO BIG)
			...
			outputText(" as Tamani does her best to catch it in her mouth and soaked cunt.<br><br>");
	...
	function tamaniFirstTimeRefusal(): void {
		//TODO Tamani Sprite spriteSelect(56);
		clearOutput();
		outputText("Tamani's eyes widen in surprise, \"<i>Don't let the size fool you, big " + player.mf("boy", "girl") + ". I can take more than you think,</i>\" she says while her hands begins playing with her box, \"<i>Are you sure you don't want to just let off a little steam?</i>\"<br><br>");
		//[Fuck Her (Goes to fuck her - consensual first time)]
		//[No means no]
		player.changeLust(5);
		addButton(0, "Fuck Her", tamaniFirstTimeConsentual);
		addButton(1, "Refuse", tamaniSecondRefusal);
	}
	...
	function tamaniSecondRefusal(): void {
		//spriteSelect(56); TODO Tamani Sprite
		clearOutput();
		outputText("The goblin pouts, anger clouding her cute little features.  She turns and storms off, clearly pissed at you, \"<i>Think about it.  Next time that dick better ache for me, or I'll MAKE you want it.</i>\"<br><br>");
		outputText("...What?");
		doNext(Camp.returnToCampUseOneHour);
	}
	...
	function tamaniMaleRepeatEncounter(): void {
		...
		if (tamanipreg.isPregnant() || gameFlags[TAMANI_NUMBER_OF_DAUGHTERS] > 0) outputText("While exploring, you're startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani there, grinning wolfishly,  \"<i>Ready for another fuck, big " + player.mf("boy", "girl") + "?\"<br><br>");
		// Else check for hypnoslut scene (large hypnosis number, then 50% chance)
		else outputText("While exploring, you're startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani the goblin there, grinning with desire, \"<i>Ready to stuff me with cum?  I'm not taking no for an answer this time.</i>\"<br><br>");
		if (gameFlags[TAMANI_TIMES_HYPNOTIZED] > 19 && rand(2) == 0) {
			tamaniHypnosis();
			return;
		}
	...
	function tamaniPregnantFuck(): void {
		...
		//(Very large:
		if (gameFlags[TAMANI_TIMES_IMPREGNATED] > 6) outputText("  They're almost too big for you to handle; they probably make it hard for the poor girl to walk.");
		outputText("\n\n");
	...
	function tamaniBirthScene(): void {
		...
		if (gameFlags[TAMANI_NUMBER_OF_DAUGHTERS] > 0) outputText("with her daughters, "); //She always has at least two daughters if she has any
		...
		tamaniGivesBirth(); //Clears pregnancies
		doNext(Camp.returnToCampUseOneHour);
	}
