namespace Areas {
	export namespace Desert {
		export function explore(): void {
			...
			const select          = choice[rand(choice.length)];
			switch (select) {
				case 0: //Sand Witch
					// Check for birthing scene. Sand Witch must be in second half of pregnancy and hit a 1/4 chance.

					if (SandWitchScene.sandwitchPreg.pregnancyEventNum == 2 && rand(4) == 0) {
						if (SandWitchScene.sandwitchPreg.pregnancyType = PregnancyType.DRIDER_EGGS) SandWitchScene.sammitchBirthsDriders();
