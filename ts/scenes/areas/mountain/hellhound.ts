...
class Hellhound extends Monster {
	...
	static hellhoundFire(): void {
		...
		/*if (player.hasStatusAffect(StatusAffects.Web_dash_Silence) >= 0) {
		 outputText("You reach inside yourself to breathe flames, but as you ready to release a torrent of fire, it backs up in your throat, blocked by the webbing across your mouth.  It causes you to cry out as the sudden, heated force explodes in your own throat.<br>");
		 changeFatigue(10);
...
namespace HellhoundScene {
	...
	function hellhoundRapesPlayer():void {
		...
		clearOutput();
		if (player.lust >= player.maxLust()) outputText("Overcome by sexual desire, you submit to the hungry hellhound.<br><br>");
		else outputText("Too weak to continue fighting, you fall to your knees.<br><br>");
		...
		if (player.lowerBody == LowerBodyType.HOOFED) outputText("hocks and knees");
		...
		if (player.vaginas.length > 0 && player.lowerBody != LowerBodyType.NAGA) outputText("slamming his twin dicks into your " + player.vaginaDescript(0) + " and your " + player.assholeDescript() + ".  ");
	...
	/*
	 ...
	 outputText("You produce the pink crystal and hand it to the master.  As the crystal touches his hand, he gives a low growl of pleasure and holds the crystal up to his eyes.  \"<i>Ah yes, excellent!  Thank you so much for this tasty treat of power.</i>\"<br><br>");
	 //if (PC gave 2/3 Merae's crystal)
	 if (player.keyItemv1("Marae's Lethicite") == 2) {
	 outputText("He looks at the crystal for a moment, before biting off half of it and giving the other half back to you.  \"<i>This is enough for the hellfire, I think.  Go ahead and keep the rest, my favorite fan!</i>\"<br><br>");
	 ...
	 outputText("He then reaches around your waist and takes a hold of your " + player.multiCockDescriptLight() + ".  \"<i>Before we get started, let's make sure you're just right for Cremera.</i>\"  He then reforms your body to have twin doggy pricks of appropriately sized.  \"<i>Now.</i>\"<br><br>");
